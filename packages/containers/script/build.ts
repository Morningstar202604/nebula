...
const reg = process.env.REGISTRY ?? "ghcr.io/Morningstar202604"
...
const setup = async () => {
  ...
  if (list.includes("nebula")) {
    await $`docker buildx use nebula`
    return
  }
  await $`docker buildx create --name nebula --use`
}
