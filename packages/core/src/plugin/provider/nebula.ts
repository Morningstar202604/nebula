...
import type { IntegrationOAuthMethodRegistration } from "@nebula-ai/plugin/v2/effect/integration"
import { define } from "@nebula-ai/plugin/v2/effect/plugin"
import type { CredentialValue } from "@nebula-ai/sdk/v2/types"
...
const defaultServer = "https://nebula.ai/console"
const clientID = "nebula-cli"
...
export const NebulaPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "nebula",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("NebulaPlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("nebula")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load Nebula provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("nebula", (integration) => {
        integration.name = "Nebula"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "nebula", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("nebula")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("nebula")
      ...
      const item = catalog.provider.get(ProviderV2.ID.nebula)
      if (!item) return
      const hasKey = Boolean(process.env.NEBULA_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("nebula")),
