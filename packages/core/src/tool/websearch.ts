...
import { ToolFailure } from "@nebula-ai/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@nebula/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.NEBULA_WEBSEARCH_PROVIDER === "exa" || process.env.NEBULA_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.NEBULA_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("NEBULA_EXPERIMENTAL") || truthy("NEBULA_ENABLE_EXA") || truthy("NEBULA_EXPERIMENTAL_EXA"),
    enableParallel: truthy("NEBULA_ENABLE_PARALLEL") || truthy("NEBULA_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `nebula/${InstallationVersion}`,
