...
import { define } from "@nebula-ai/plugin/v2/effect"
...
import { LocationServiceMap } from "@nebula-ai/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "nebula.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "nebula.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
