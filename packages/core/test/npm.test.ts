...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@nebula/acme")).toBe("@nebula/acme")
    expect(Npm.sanitize("@nebula/acme@1.0.0")).toBe("@nebula/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/nebula/acme.git"
    const expected = win ? "acme@git+https_//github.com/nebula/acme.git" : spec
