...
import { ModelsDevPlugin } from "@nebula-ai/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.NEBULA_MODELS_PATH,
          disabled: Flag.NEBULA_DISABLE_MODELS_FETCH,
        }
        Flag.NEBULA_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
        Flag.NEBULA_DISABLE_MODELS_FETCH = true
        return previous
      }),
      () =>
        Effect.gen(function* () {
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.NEBULA_MODELS_PATH = previous.path
          Flag.NEBULA_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
