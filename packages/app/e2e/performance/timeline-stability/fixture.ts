...
import { Event } from "@nebula-ai/schema/event"
import { SessionStatusEvent } from "@nebula-ai/schema/session-status-event"
import { SessionV1 } from "@nebula-ai/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
export const directory = "C:/Nebula/TimelineStability"
...
export const model = { providerID: "nebula", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("nebula.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
...
function provider() {
  return {
    ...
    connected: ["nebula"],
    default: { providerID: "nebula", modelID: "claude-opus-4-6" },
  }
}
