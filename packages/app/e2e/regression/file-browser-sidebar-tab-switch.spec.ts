...
const directory = "C:/Nebula/FileBrowserSidebar"
...
async function setup(
  ...
  await mockNebulaServer(page, {
    ...
    provider: {
      ...
      connected: ["nebula"],
      default: { providerID: "nebula", modelID: "test" },
    },
    sessions: [
      {
        id: sessionID,
  ...
  await page.addInitScript(
    ({ directory, server, sessionID }) => {
      ...
      localStorage.setItem(
        "nebula.global.dat:layout",
        JSON.stringify({ review: { diffStyle: "split", panelOpened: true } }),
      )
      localStorage.setItem(
        "nebula.global.dat:review-panel-v2",
        JSON.stringify({ sidebarOpened: true, sidebarWidth: 240, expandMode: "collapse" }),
      )
      localStorage.setItem(
        "nebula.window.browser.dat:tabs",
