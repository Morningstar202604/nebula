...
import type { retry } from "@nebula-ai/core/util/retry"
import type { NebulaEvent, SessionApi } from "@nebula-ai/client/promise"
import type { Message, NebulaClient, Part, Session } from "@nebula-ai/sdk/v2/client"
...
describe("server session", () => {
  test("projects V2 session events into current and legacy message state", () => {
    ...
    const apply = (input: object) => ctx.store.applyV2(input as NebulaEvent)
  ...
  test("extends a current page to include the user for split assistant turns", async () => {
    ...
    const store = createServerSession({} as NebulaClient, {} as SessionApi, messageApi)
    store.remember(session("root"))
