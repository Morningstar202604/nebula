import type { IntegrationMethod, IntegrationOauthConnectOutput } from "@nebula-ai/client/promise"
import { Button } from "@nebula-ai/ui/button"
import { useDialog } from "@nebula-ai/ui/context/dialog"
import { Dialog } from "@nebula-ai/ui/dialog"
import { Icon } from "@nebula-ai/ui/icon"
import { IconButton } from "@nebula-ai/ui/icon-button"
import { List, type ListRef } from "@nebula-ai/ui/list"
import { ProviderIcon } from "@nebula-ai/ui/provider-icon"
import { Spinner } from "@nebula-ai/ui/spinner"
import { Tag } from "@nebula-ai/ui/tag"
import { TextField } from "@nebula-ai/ui/text-field"
import { ButtonV2 } from "@nebula-ai/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@nebula-ai/ui/v2/dialog-v2"
import { TextInputV2 } from "@nebula-ai/ui/v2/text-input-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "nebula-go") return language.t("dialog.provider.nebulaGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "nebula"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.nebula.tagline")}</div>
          </Show>
...
function ProviderPickerV2(props: {
  ...
  const featured = ["nebula", "nebula-go", "anthropic", "openai", "google", "openrouter", "vercel"]
                  ...
                  <For each={group.items()}>
                    {(provider) => (
                      ...
                      >
                        ...
                        <Show when={provider.id === "nebula" || provider.id === "nebula-go"}>
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "nebula"
                                ? "dialog.provider.nebula.tagline"
                                : "dialog.provider.nebulaGo.tagline",
                            )}
                          </span>
  ...
  function ApiAuthView() {
    ...
    if (newLayout())
      return (
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          <Show
            when={provider().id === "nebula"}
            fallback={language.t("provider.connect.apiKey.description", { provider: provider().name })}
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.nebulaZen.line1")}</div>
              <div>{language.t("provider.connect.nebulaZen.line2")}</div>
              <div>
                {language.t("provider.connect.nebulaZen.visit.prefix")}
                <ExternalLink
                  href="https://nebula.ai/zen"
                  class="text-v2-text-text-base focus-visible:rounded-xs focus-visible:outline-2 focus-visible:outline-v2-border-border-focus"
                >
                  {language.t("provider.connect.nebulaZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.nebulaZen.visit.suffix")}
              </div>
            </div>
          </Show>
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "nebula"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.nebulaZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.nebulaZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.nebulaZen.visit.prefix")}
                <ExternalLink href="https://nebula.ai/zen" tabIndex={-1}>
                  {language.t("provider.connect.nebulaZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.nebulaZen.visit.suffix")}
              </div>
