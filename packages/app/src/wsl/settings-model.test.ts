...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when Nebula needs attention", () => {
    expect(wslNebulaAction(undefined)).toBeUndefined()
    expect(
      wslNebulaAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    ).toBe("wsl.onboarding.installNebula")
    expect(
      wslNebulaAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/nebula",
        version: "1.2.2",
        expectedVersion: "1.2.3",
        matchesDesktop: false,
        error: null,
      }),
    ).toBe("wsl.onboarding.updateNebula")
    expect(
      wslNebulaAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/nebula",
  ...
  test("does not report ready when Nebula is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        nebulaChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.nebula/bin/nebula",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
            error: "nebula is installed but could not run",
          },
        },
      },
      view: "main",
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installNebula" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-nebula")
  })
