...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-nebula" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wslNebulaAction(check?: WslNebulaCheck) {
  if (!check) return
  if (!check.resolvedPath) return "wsl.onboarding.installNebula"
  if (check.matchesDesktop === false) return "wsl.onboarding.updateNebula"
}
...
export function addServerViewModel(input: {
  ...
  const nebulaCheck = selectedDistro ? (state?.nebulaChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.nebulaChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateNebula" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.nebulaMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installNebula" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  nebulaCheck: WslNebulaCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const installingNebula =
    input.state?.job?.kind === "install-nebula" && input.state.job.distro === input.selectedDistro
  if (!ready || probingNebula) {
    return {
      variant: "contrast",
  ...
  if (!addServerNebulaReady(input.nebulaCheck)) {
    const update = !!input.nebulaCheck?.resolvedPath && input.nebulaCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingNebula
        ? { key: "wsl.onboarding.updatingNebula" }
        : update
          ? { key: "wsl.onboarding.updateNebula" }
          : { key: "wsl.onboarding.installNebula" },
      disabled: !!input.state?.job || input.adding,
      action: "install-nebula",
      loading: installingNebula,
...
function addServerNebulaReady(check: WslNebulaCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.nebulaChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.nebulaChecks[item.name]) return [`nebula:${item.name}`]
    return []
  })
