...
import { Tag } from "@nebula-ai/ui/v2/badge-v2"
...
import { wslNebulaAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.nebulaChecks[item.config.distro]
          const nebulaAction = () => wslNebulaAction(check())
          const busy = () => wsl.data?.job?.kind === "install-nebula" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={nebulaAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installNebula(item.config.distro))}
                    >
