...
import { type Session } from "@nebula-ai/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  compareSessionTime,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("nebula://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("nebula://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("nebula://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("nebula://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("nebula://open-project")).toBeUndefined()
    expect(parseDeepLink("nebula://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "nebula://open-project?directory=/a",
      "nebula://other?directory=/b",
      "nebula://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("nebula://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("nebula://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("nebula://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("nebula://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "nebula://new-session?directory=/a",
      "nebula://open-project?directory=/b",
      "nebula://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __NEBULA__: {
        deepLinks: ["nebula://open-project?directory=/a"],
      },
    } as unknown as Window & { __NEBULA__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["nebula://open-project?directory=/a"])
