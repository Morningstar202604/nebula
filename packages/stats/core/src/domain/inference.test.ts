...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve nebula route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "nebula")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "nebula")).toBe("openai")
    expect(statProvider("big-pickle", "", "nebula")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "nebula"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep nebula as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "nebula"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "nebula"))).toMatchObject([{ provider: "unknown" }])
    expect(toProviderAggregate(aggregate("muse-spark-1.2-contributor", "unknown"))).toMatchObject([
      { provider: "meta" },
    ])
  })

  test("geo aggregates never keep nebula or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "nebula"), country: "US" })).toMatchObject([
