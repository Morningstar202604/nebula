...
export const NEBULA_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const NEBULA_MODE_KEY = "nebula.mode"

export const NebulaKeymapProvider = KeymapProvider
export const useNebulaKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type NebulaModeStack = ReturnType<typeof createNebulaModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, NebulaModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createNebulaModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(NEBULA_MODE_KEY, NEBULA_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(NEBULA_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(NEBULA_MODE_KEY, stack.at(-1)?.mode ?? NEBULA_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? NEBULA_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(NEBULA_MODE_KEY, undefined)
...
export function useNebulaModeStack() {
  return getNebulaModeStack(useNebulaKeymap())
}

export function getNebulaModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Nebula mode stack is not registered for this keymap")
  return value
}
...
export function registerNebulaKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createNebulaModeStack(keymap)
