import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@nebula-ai/plugin/tui"
...
import type { useNebulaKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useNebulaKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getNebulaModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getNebulaModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
