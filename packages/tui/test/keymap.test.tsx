...
import { getNebulaModeStack, NEBULA_BASE_MODE, NebulaKeymapProvider, registerNebulaKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerNebulaKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when nebula mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getNebulaModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getNebulaModeStack(keymap).push("autocomplete")
