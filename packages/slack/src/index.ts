...
import { createNebula, type ToolPart } from "@nebula-ai/sdk"
...
console.log("🚀 Starting nebula server...")
const nebula = await createNebula({
  port: 0,
})
console.log("✅ Nebula server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await nebula.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new nebula session...")
    const { client, server } = nebula
    ...
    console.log("✅ Created nebula session:", createResult.data.id)
  ...
  console.log("📝 Sending to nebula:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Nebula response:", JSON.stringify(result, null, 2))
