...
import type { UpdaterState } from "@nebula-ai/app/updater"
...
import "./styles.css"
import { Splash } from "@nebula-ai/ui/logo"
import { useTheme } from "@nebula-ai/ui/theme/context"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.NEBULA_CHANNEL === "prod" &&
...
const deepLinkEvent = "nebula:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__NEBULA__ ??= {}
  const pending = window.__NEBULA__.deepLinks ?? []
  window.__NEBULA__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `nebula.desktop.window.${windowID}.last-active-url`
}
  ...
  return {
    ...
    notify: async (title, description, onClick) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://nebula.ai/favicon-96x96-v3.png",
      })
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("nebula.global.dat").getItem("language")
