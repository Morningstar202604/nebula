...
let releaseNebulaResolve: (() => void) | undefined
...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectNebulaVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectNebulaVersion("1.14.35", "1.16.2")).toThrow(
    "Nebula update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating Nebula", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.nebula/bin/nebula",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, nebulaChecks: {} })
})
...
test("ignores stale background Nebula checks after removing a WSL server", async () => {
  persistedServers = []
  releaseNebulaResolve = undefined
  ...
  await waitFor(() => !!releaseNebulaResolve)
  await controller.removeServer("wsl:Debian")
  releaseNebulaResolve?.()
  await new Promise((resolve) => setTimeout(resolve, 0))

  expect(controller.getState().servers).toEqual([])
  expect(controller.getState().nebulaChecks).toEqual({})
})

test("ignores stale startup Nebula checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking Nebula", async () => {
  persistedServers = []
  ...
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolveNebula: async (distro) => {
      nebula.push(distro)
      return "/home/me/.nebula/bin/nebula"
    },
  })
  ...
  expect(nebula).toEqual([])
  release.get("Debian")?.()
  release.get("Ubuntu")?.()
  await task

  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(nebula).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().nebulaChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check Nebula in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const nebula: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(nebula).toEqual(["Debian"])
  expect(Object.keys(controller.getState().nebulaChecks)).toEqual(["Debian"])
})
