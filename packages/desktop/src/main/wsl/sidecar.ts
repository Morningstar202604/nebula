...
import { type WslCommandLine, resolveWslNebula, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const nebula = await resolveWslNebula(distro)
  if (!nebula) throw new Error(nativeT("desktop.wsl.error.nebulaNotInstalled", { distro }))

  const port = await allocatePort()
  const password = randomUUID()
  const username = "nebula"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export NEBULA_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export NEBULA_CLIENT=desktop",
    `export NEBULA_SERVER_USERNAME=${shellEscape(username)}`,
    `export NEBULA_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(nebula)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
