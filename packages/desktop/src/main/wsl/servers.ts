...
import { expectNebulaVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
...
type WslServersControllerOptions = {
  ...
  resolveNebula?: typeof resolveWslNebula
...
export function createWslServersController(
  ...
  const setNebulaCheck = (distro: string, check: WslNebulaCheck) => {
    setState({
      nebulaChecks: {
        ...state.nebulaChecks,
        [distro]: check,
  ...
  const checkNebula = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolveNebula ?? resolveWslNebula)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return nebulaCheck(distro, resolved, version, appVersion)
  }

  const refreshNebulaCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setNebulaCheck(distro, await checkNebula(distro, opts))
  }
  ...
  const probeAddableDistros = async (distros: string[], opts?: { signal?: AbortSignal }) => {
    ...
    const nebulaChecks = await Promise.all(
      unique
        .filter((distro) => distroProbeReady(state.distroProbes[distro]))
        .filter((distro) => !state.nebulaChecks[distro])
        .map(async (distro) => [distro, await checkNebula(distro, opts)] as const),
    )
    if (nebulaChecks.length) {
      setState({ nebulaChecks: { ...state.nebulaChecks, ...Object.fromEntries(nebulaChecks) } })
    }
  }
  ...
  const refreshNebulaCheckBackground = (id: string, distro: string) => {
    void checkNebula(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setNebulaCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl nebula check failed", { id, distro, message })
      })
  }

  const refreshNebulaChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkNebula(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setNebulaCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl nebula check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshNebulaCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async installNebula(name: string) {
      await runJob({ kind: "install-nebula", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslNebula(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || nativeT("desktop.wsl.error.installNebula"))
        }
        await refreshNebulaCheck(name, { signal: abort.signal })
        expectNebulaVersion(state.nebulaChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.nebulaChecks, distro) : {}),
      })
    },
...
function nebulaCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: nativeT("desktop.wsl.error.nebulaMissing"),
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: nativeT("desktop.wsl.error.nebulaCannotRun"),
    }
  }
  return {
    distro,
