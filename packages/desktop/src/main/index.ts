...
const TEST_ONBOARDING = process.env.NEBULA_TEST_ONBOARDING === "1"
const SIDECAR_VERSION = process.env.NEBULA_SIDECAR_V2 === "1" ? "v2" : "v1"
...
const main = Effect.gen(function* () {
  ...
  process.env.NEBULA_DISABLE_EMBEDDED_WEB_UI = "true"

  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "ai.nebula.desktop.dev"
  const onboardingTestRoot = ((): string | undefined => {
    if (!TEST_ONBOARDING) return

    const root = join(tmpdir(), `nebula-onboarding-${randomUUID()}`)
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "Nebula Dev")
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("nebula://"))
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient("nebula")
  registerRendererProtocol()
  setDockIcon()
  ...
  const loadingTask = yield* Effect.gen(function* () {
    ...
    const port = yield* Effect.gen(function* () {
      const fromEnv = process.env.NEBULA_PORT
      if (fromEnv) {
