...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.nebula.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `nebula.settings.dat` is special: it maps to the `nebula.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"nebula.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // nebula.settings.dat → the electron settings store ("nebula.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "nebula.global.dat").
  const storeName = filename === "nebula.settings.dat" ? "nebula.settings" : filename
