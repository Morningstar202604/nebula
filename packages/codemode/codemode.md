...
This is the living design and status document for `@nebula-ai/codemode` and its existing V2 Nebula adapter.
...
`packages/nebula/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
