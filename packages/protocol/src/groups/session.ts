...
import { PromptInput } from "@nebula-ai/schema/prompt-input"
import { Session } from "@nebula-ai/schema/session"
import { Project } from "@nebula-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@nebula-ai/schema/schema"
...
import { SessionEvent } from "@nebula-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this Nebula process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this Nebula process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
