...
> current private `@nebula-ai/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@nebula-ai/ai`
...
- Secondary audience: Nebula and other durable agent runtimes
...
import { LLM } from "@nebula-ai/ai"
import { OpenAI } from "@nebula-ai/ai/providers/openai"
...
```ts
import { Message } from "@nebula-ai/ai"
...
import { LLM, Tool } from "@nebula-ai/ai"
...
Nebula and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@nebula-ai/ai/promise"
import { OpenAI } from "@nebula-ai/ai/providers/openai"
...
```ts
import { LLMSchema } from "@nebula-ai/ai/schema"
...
```ts
import { Provider, Protocol } from "@nebula-ai/ai/provider"
...
@nebula-ai/ai/promise
  Promise/AsyncIterable LLM facade

@nebula-ai/ai/schema
  serializable domain schemas

@nebula-ai/ai/provider
  experimental Provider and Protocol authoring APIs

@nebula-ai/ai/providers/openai
@nebula-ai/ai/providers/anthropic
@nebula-ai/ai/providers/google
...
```
...
Nebula should migrate to `generateTurn` / `streamTurn`, preserving its durable
