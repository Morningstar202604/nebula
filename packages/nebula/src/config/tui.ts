...
import { isRecord } from "@nebula-ai/tui/util/record"
...
import { TuiKeybind } from "@nebula-ai/tui/config/keybind"
...
import { TuiConfig } from "@nebula-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@nebula/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old nebula.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.nebula`
  // folders between cwd and home, and NEBULA_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.NEBULA_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit NEBULA_TUI_CONFIG override, if set.
  if (Flag.NEBULA_TUI_CONFIG) {
    const configFile = Flag.NEBULA_TUI_CONFIG
  ...
  // 4. `.nebula` directories (and NEBULA_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".nebula") || dir === Flag.NEBULA_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".nebula") && dir !== Flag.NEBULA_CONFIG_DIR) continue
