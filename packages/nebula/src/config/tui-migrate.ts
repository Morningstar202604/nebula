...
const TUI_SCHEMA_URL = "https://nebula.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from nebula.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const nebula = await nebulaFiles(input)
  for (const file of nebula) {
...
async function nebulaFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "nebula"),
    ...(await Filesystem.findUp(["nebula.json", "nebula.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "nebula"))
  }
  if (Flag.NEBULA_CONFIG) files.push(Flag.NEBULA_CONFIG)
