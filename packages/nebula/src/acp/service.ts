...
import type { AssistantMessage, Message, NebulaClient, SessionMessageResponse } from "@nebula-ai/sdk/v2"
...
export const AuthMethodID = "nebula-login"
...
export class Service extends Context.Service<Service, Interface>()("@nebula/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `nebula auth login` in the terminal",
      name: "Login with nebula",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: NebulaClient) {
...
function makeUsageService(sdk: NebulaClient) {
...
async function loadDirectorySnapshot(sdk: NebulaClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, nebula provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const nebulaProvider = providers[ProviderV2.ID.make("nebula")]
  const nebulaModel = nebulaProvider ? Provider.sort(Object.values(nebulaProvider.models))[0] : undefined
  if (nebulaProvider && nebulaModel) return { providerID: nebulaProvider.id, modelID: nebulaModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Nebula service failure", service })
}
