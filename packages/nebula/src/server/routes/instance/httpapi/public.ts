...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for nebula serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(NebulaHttpApi)
...
export const PublicApi = NebulaHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "nebula",
    version: "1.0.0",
