...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@nebula-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@nebula-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@nebula-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@nebula-ai/tui/context/theme"
import { Global } from "@nebula-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@nebula-ai/core/util/flock"
import { Flag } from "@nebula-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@nebula-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@nebula-ai/core/v1/config/plugin"
import { createCommandShim } from "@nebula-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@nebula-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".nebula"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".nebula", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".nebula", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.NEBULA_PURE ? [] : pluginOrigins
    if (Flag.NEBULA_PURE && pluginOrigins.length) {
    }
