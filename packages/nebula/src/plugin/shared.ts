...
export const DEPRECATED_PLUGIN_PACKAGES = ["nebula-openai-codex-auth", "nebula-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, nebulaVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(nebulaVersion) || semver.major(nebulaVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.nebula
  if (typeof range !== "string") return
  if (!semver.satisfies(nebulaVersion, range)) {
    throw new Error(`Plugin requires nebula ${range} but running ${nebulaVersion}`)
  }
}
