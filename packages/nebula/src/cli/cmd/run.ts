import type { PermissionV1 } from "@nebula-ai/core/v1/permission"
import { FSUtil } from "@nebula-ai/core/fs-util"
// CLI entry point for `nebula run` and `nebula --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`nebula --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`nebula --mini --attach`): connects to a running
//      nebula server and runs interactive mode against it.
//
...
import { createNebulaClient, type NebulaClient, type ToolPart } from "@nebula-ai/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<NebulaClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run nebula with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running nebula server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to NEBULA_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to NEBULA_SERVER_USERNAME or 'nebula')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: NebulaClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: NebulaClient, sessionID: string) {
      ...
      async function current(sdk: NebulaClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: NebulaClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: NebulaClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: NebulaClient) {
        ...
        async function loop(client: NebulaClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createNebulaClient({
        baseUrl: "http://nebula.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
