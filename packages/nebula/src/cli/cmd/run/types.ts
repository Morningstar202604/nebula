// Shared type vocabulary for the direct interactive mode (`nebula --mini`).
//
...
import type { NebulaClient, PermissionRequest, QuestionRequest, ToolPart } from "@nebula-ai/sdk/v2"
import type { TuiConfig } from "@nebula-ai/tui/config"
...
type PromptModel = Parameters<NebulaClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<NebulaClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<NebulaClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<NebulaClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<NebulaClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<NebulaClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<NebulaClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<NebulaClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<NebulaClient["question"]["reject"]>[0]
