...
import type { Event, NebulaClient } from "@nebula-ai/sdk/v2"
...
type PermissionReplyParams = Parameters<NebulaClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "nebula-acp-permission-"))
