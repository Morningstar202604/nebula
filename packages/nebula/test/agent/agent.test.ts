...
it.instance("plan agent denies edits except .nebula/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".nebula/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".nebula", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.NEBULA_TEST_HOME
      process.env.NEBULA_TEST_HOME = test.directory
