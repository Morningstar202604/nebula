...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "nebula.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "nebula.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "nebula.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "nebula.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["nebula.json", "nebula.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "nebula.json"),
        path.join(project, "nebula.jsonc"),
      ])
    })
  })
