...
import { EventSequenceTable } from "@nebula-ai/core/event/sql"
...
import { WorkspaceV2 } from "@nebula-ai/core/workspace"
import { WorkspaceTable } from "@nebula-ai/core/control-plane/workspace.sql"
...
import { Ripgrep } from "@nebula-ai/core/ripgrep"
import { AppNodeBuilder } from "@nebula-ai/core/effect/app-node-builder"
import { LayerNode } from "@nebula-ai/core/effect/layer-node"

const originalEnv = {
  NEBULA_AUTH_CONTENT: process.env.NEBULA_AUTH_CONTENT,
  NEBULA_EXPERIMENTAL_WORKSPACES: process.env.NEBULA_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.NEBULA_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@nebula.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.NEBULA_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=nebula-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.NEBULA_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.NEBULA_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.NEBULA_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=nebula-test")
