...
import { LLMClient, RequestExecutor } from "@nebula-ai/llm/route"
...
describe("session.llm.stream", () => {
  ...
  const nebulaFixture = { providerID: "nebula-test", modelID: vivgridFixture.modelID }

  it.instance(
    "sends the parent session header for nebula providers",
    () =>
      Effect.gen(function* () {
        ...
        const resolved = yield* Provider.use.getModel(
          ProviderV2.ID.make(nebulaFixture.providerID),
          ModelV2.ID.make(nebulaFixture.modelID),
        )
        ...
        const user = {
          ...
          agent: agent.name,
          model: {
            providerID: ProviderV2.ID.make(nebulaFixture.providerID),
            modelID: resolved.id,
          },
      ...
      }),
    {
      config: () => {
        const fixture = loadFixture(vivgridFixture.providerID, vivgridFixture.modelID)
        return {
          enabled_providers: [nebulaFixture.providerID],
          provider: {
            [nebulaFixture.providerID]: {
              name: "Nebula Test",
