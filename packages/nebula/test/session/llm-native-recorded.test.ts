...
import { SessionV1 } from "@nebula-ai/core/v1/session"
import { ModelsDev } from "@nebula-ai/core/models-dev"
import { HttpRecorder } from "@nebula-ai/http-recorder"
import { HttpRecorderInternal } from "@nebula-ai/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@nebula-ai/llm"
import { RequestExecutor } from "@nebula-ai/llm/route"
...
const zenURL = (connection: string) => `https://console.nebula.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.NEBULA_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["nebula", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("NEBULA_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("NEBULA_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["nebula", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "nebula-proxy",
    name: "Nebula proxy",
    providerID: ProviderV2.ID.nebula,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["nebula", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.NEBULA_RECORD_CONSOLE_TOKEN && process.env.NEBULA_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.nebula,
        name: "Nebula Zen",
        env: ["NEBULA_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.NEBULA_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.NEBULA_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.NEBULA_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["nebula", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("NEBULA_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("NEBULA_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("NEBULA_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set NEBULA_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the nebula LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "nebula.json"),
      JSON.stringify({ $schema: "https://nebula.ai/config.json", ...scenario.config(model) }),
    ),
  )
