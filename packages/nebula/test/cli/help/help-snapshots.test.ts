...
// terminal sizes. The default nebula tui command is excluded —
// `nebula --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `nebula --help` prints today;
...
// top-level help on `--help` and exits 1; not a real nebula command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("nebula CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ nebula }) =>
      Effect.gen(function* () {
        const topLevel = yield* nebula.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* nebula.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`nebula ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`nebula ${argv.join(" ")} --help`)
        }
