// Subprocess integration tests for `nebula run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `nebula.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `NEBULA_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("nebula run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, nebula }) =>
      Effect.gen(function* () {
        ...
        const result = yield* nebula.run("say hi")
        nebula.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, nebula }) =>
      Effect.gen(function* () {
        ...
        const result = yield* nebula.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, nebula }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* nebula.run("think", { extraArgs: ["--thinking"] })
        nebula.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* nebula.run("think again")
        nebula.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, nebula }) =>
      Effect.gen(function* () {
        ...
        const result = yield* nebula.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, nebula }) =>
      Effect.gen(function* () {
        ...
        const result = yield* nebula.run("say hi", { format: "json" })
        nebula.expectExit(result, 0)

        const events = nebula.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ nebula }) =>
      Effect.gen(function* () {
        const result = yield* nebula.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, nebula }) =>
      Effect.gen(function* () {
        ...
        const result = yield* nebula.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, nebula }) =>
      Effect.gen(function* () {
        ...
        yield* llm.text("recovered")
        const result = yield* nebula.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, nebula }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* nebula.run("request permission", { permission: { bash: "ask" } })
        nebula.expectExit(denied, 0)
        ...
        const allowed = yield* nebula.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        nebula.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* nebula.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        nebula.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, nebula }) =>
      Effect.gen(function* () {
        ...
        const server = yield* nebula.serve()

        const result = yield* nebula.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, nebula }) =>
      Effect.gen(function* () {
        const result = yield* nebula.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, nebula }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* nebula.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
