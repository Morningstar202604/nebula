...
import { NebulaClient, type GlobalEvent } from "@nebula-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<NebulaClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<NebulaClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<NebulaClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<NebulaClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<NebulaClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://nebula.test"),
...
function sdk(
  input: {
    ...
    subscribe?: NebulaClient["event"]["subscribe"]
    globalEvent?: NebulaClient["global"]["event"]
    promptAsync?: NebulaClient["session"]["promptAsync"]
    status?: NebulaClient["session"]["status"]
    messages?: NebulaClient["session"]["messages"]
    children?: NebulaClient["session"]["children"]
    permissions?: NebulaClient["permission"]["list"]
    questions?: NebulaClient["question"]["list"]
  } = {},
) {
  const client = new NebulaClient()

  const subscribe: NebulaClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: NebulaClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: NebulaClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: NebulaClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: NebulaClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: NebulaClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: NebulaClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: NebulaClient["question"]["list"] = input.questions ?? (() => ok([]))
