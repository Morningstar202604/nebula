...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ nebula }) =>
    Effect.gen(function* () {
      const result = yield* nebula.spawn(["--replay-limit", "10"])

      nebula.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ nebula }) =>
    Effect.gen(function* () {
      const result = yield* nebula.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ nebula }) =>
    Effect.gen(function* () {
      const result = yield* nebula.spawn(["--mini", "--port", "4096"])
