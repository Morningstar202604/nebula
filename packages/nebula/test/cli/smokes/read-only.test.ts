...
describe("nebula read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // NEBULA_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ nebula }) =>
      Effect.gen(function* () {
        const r = yield* nebula.spawn(["mcp", "list"])
        nebula.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ nebula }) =>
      Effect.gen(function* () {
        const r = yield* nebula.spawn(["providers", "list"])
        nebula.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ nebula }) =>
      Effect.gen(function* () {
        const r = yield* nebula.spawn(["models"])
        nebula.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ nebula }) =>
      Effect.gen(function* () {
        const r = yield* nebula.spawn(["agent", "list"])
        nebula.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh NEBULA_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ nebula }) =>
      Effect.gen(function* () {
        const r = yield* nebula.spawn(["session", "list"])
        nebula.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ nebula }) =>
      Effect.gen(function* () {
        const r = yield* nebula.spawn(["stats"])
        nebula.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under NEBULA_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ nebula }) =>
      Effect.gen(function* () {
        const r = yield* nebula.spawn(["db", "path"])
        nebula.expectExit(r, 0, "db path")
