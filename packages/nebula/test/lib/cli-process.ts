// Subprocess test harness for the nebula CLI. Spawns the real binary against
...
//
// Configuration flows through nebula's built-in test affordances:
//   - NEBULA_CONFIG_CONTENT      : provider config inline, no files to find
//   - NEBULA_TEST_HOME           : pins os.homedir() → tmpdir
//   - NEBULA_DISABLE_PROJECT_CONFIG : skip walking up for nebula.json
//   - NEBULA_PURE                : skip external plugin discovery + install
//   - NEBULA_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `nebula.run` is fully wired. The shape supports adding more
// builders (`nebula.serve(opts)`, `nebula.acp(opts)`, `nebula.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of NebulaCli.
...
import { AppProcess } from "@nebula-ai/core/process"
...
const nebulaRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(nebulaRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    NEBULA_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    NEBULA_CONFIG_CONTENT: configJson,
    NEBULA_DISABLE_PROJECT_CONFIG: "1",
    NEBULA_PURE: "1",
    NEBULA_DISABLE_AUTOUPDATE: "1",
    NEBULA_DISABLE_AUTOCOMPACT: "1",
    NEBULA_DISABLE_MODELS_FETCH: "1",
    NEBULA_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `nebula run`. New flags should
...
// `nebula serve` is a long-lived process — it never exits on its own.
...
// `nebula acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type NebulaCli = {
  ...
  // Spawn `nebula serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `nebula acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("nebula.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          NEBULA_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("nebula.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("nebula.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "nebula server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `nebula serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("nebula.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const nebula: NebulaCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, nebula })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `nebula.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "nebula") {
...
// every call site. Use this for any test that needs the nebula CLI fixture.
//
...
// resources (e.g. `nebula.serve`) without an extra `Effect.scoped` wrapper —
