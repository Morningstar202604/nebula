...
Practical reference for a future `packages/server` split after the nebula
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/nebula`.
...
Until enough shared service code lives outside `packages/nebula`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/nebula`

It should not import `packages/nebula` services while `packages/nebula`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on nebula-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/nebula` implementation details.
