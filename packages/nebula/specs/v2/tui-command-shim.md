...
Delete `packages/nebula/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/nebula/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/nebula/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/nebula`
- TUI plugin loader tests in `packages/nebula` if runtime plugin API wiring changed
