...
Trim `packages/nebula/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/nebula/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/nebula`.
- `[x]` `bun typecheck` from `packages/nebula`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/nebula/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/nebula`.
- `[x]` `bun dev generate > /tmp/nebula-openapi.json` from `packages/nebula`.
...
- Edit `packages/nebula/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/nebula-openapi.json` from `packages/nebula`.
...
- Check source schemas in files like `packages/nebula/src/session/schema.ts`, `packages/nebula/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/nebula/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/nebula/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
