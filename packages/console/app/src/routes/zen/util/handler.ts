...
import { and, Database, eq, isNull, lt, or, sql } from "@nebula-ai/console-core/drizzle/index.js"
import { KeyTable } from "@nebula-ai/console-core/schema/key.sql.js"
import { BillingTable, LiteTable, SubscriptionTable, UsageTable } from "@nebula-ai/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@nebula-ai/console-core/util/price.js"
import { getMonthlyBounds, getWeekBounds } from "@nebula-ai/console-core/util/date.js"
import { Identifier } from "@nebula-ai/console-core/identifier.js"
import { Billing } from "@nebula-ai/console-core/billing.js"
import { Actor } from "@nebula-ai/console-core/actor.js"
import { WorkspaceTable } from "@nebula-ai/console-core/schema/workspace.sql.js"
import { ZenData } from "@nebula-ai/console-core/model.js"
import { Subscription } from "@nebula-ai/console-core/subscription.js"
import { BlackData } from "@nebula-ai/console-core/black.js"
import { UserTable } from "@nebula-ai/console-core/schema/user.sql.js"
import { ModelTable } from "@nebula-ai/console-core/schema/model.sql.js"
import { ProviderTable } from "@nebula-ai/console-core/schema/provider.sql.js"
...
import { LiteData } from "@nebula-ai/console-core/lite.js"
import { Resource } from "@nebula-ai/console-resource"
...
import { Workspace } from "@nebula-ai/console-core/workspace.js"
...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-nebula-session") ?? ""
    const requestId = input.request.headers.get("x-nebula-request") ?? ""
    const ocClient = input.request.headers.get("x-nebula-client") ?? ""
    const projectId = input.request.headers.get("x-nebula-project") ?? ""
    ...
    if (
      ...
      throw new DataPolicyError(
        t("zen.api.error.trainingNotAllowed", {
          consoleGoUrl: `https://nebula.ai/workspace/${authInfo.workspaceID}/go`,
        }),
      )
    ...
    const providerRequest = async () => {
      ...
      const res = await fetch(reqUrl, {
        ...
        headers: (() => {
          ...
          if (!isNewInference) {
            headers.delete("x-nebula-session")
            headers.delete("x-nebula-project")
            headers.delete("x-nebula-client")
            headers.delete("x-nebula-request")
      ...
      if (isNewInference) {
        const resEndpointId = res.headers.get("x-nebula-endpoint-id")
        const resEndpointModelId = res.headers.get("x-nebula-upstream-model-id")
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      ...
      try {
        const consoleGoUrl = `https://nebula.ai/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://nebula.ai/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://nebula.ai/workspace/${authInfo.workspaceID}/members`
