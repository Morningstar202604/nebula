...
describe("authorization redirect validation", () => {
  test("allows registered Nebula callbacks", () => {
    expect(isAllowedAuthorizationRedirect("app", "https://nebula.ai/auth/callback")).toBe(true)
    expect(isAllowedAuthorizationRedirect("app", "https://dev.nebula.ai/auth/callback")).toBe(true)
  ...
  test("rejects unregistered clients and external redirects", () => {
    expect(isAllowedAuthorizationRedirect("other", "https://nebula.ai/auth/callback")).toBe(false)
    expect(isAllowedAuthorizationRedirect("app", "https://evil.example/callback")).toBe(false)
    expect(isAllowedAuthorizationRedirect("app", "https://nebula.ai.evil.example/callback")).toBe(false)
