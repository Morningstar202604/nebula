...
Tools allow the LLM to perform actions in your codebase. Nebula comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in Nebula.
...
:::note
This tool is only available when `NEBULA_EXPERIMENTAL_LSP_TOOL=true` (or `NEBULA_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the Nebula or Nebula Go provider, or when either the `NEBULA_ENABLE_EXA` or `NEBULA_ENABLE_PARALLEL` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching Nebula:

```bash
NEBULA_ENABLE_EXA=1 nebula
# or
NEBULA_ENABLE_PARALLEL=1 nebula
```
