...
Le skill degli agenti permettono a Nebula di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Nebula cerca in queste posizioni:

- Config di progetto: `.nebula/skills/<name>/SKILL.md`
- Config globale: `~/.config/nebula/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Nebula risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.nebula/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/nebula/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.nebula/skills/git-release/SKILL.md` cosi':
...
Nebula elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `nebula.json`:
...
**Per agenti integrati** (in `nebula.json`):
