...
Puoi aggiungere strumenti esterni a Nebula usando il _Model Context Protocol_, o MCP. Nebula supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Nebula Config](https://nebula.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/nebula`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Nebula gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Nebula:
...
Se il server richiede autenticazione, Nebula ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `nebula mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Nebula memorizzera' i token in modo sicuro in `~/.local/share/nebula/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Nebula insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Nebula come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Nebula al tuo account Sentry.
