---
title: Server
description: Interagisci con il server nebula via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `nebula serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client nebula.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `nebula.local` |
...
Imposta `NEBULA_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `nebula`, oppure imposta `NEBULA_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `nebula serve` sia per `nebula web`.
...
Quando esegui `nebula` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server nebula per interagire con nebula in modo programmatico.
:::

Questa architettura permette a nebula di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `nebula serve` per avviare un server standalone. Se la TUI di nebula e gia in esecuzione, `nebula serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Nebula.
...
Il server nebula espone le seguenti API.
