...
Nebula Go è un abbonamento a basso costo da **10 $/mese** che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in Nebula. Ti abboni a Nebula Go e
...
Nebula Go ti dà accesso a questi modelli a **10 $/mese**.

---

## Come funziona

Nebula Go funziona come qualsiasi altro provider in Nebula.

1. Accedi a **<a href={console}>Nebula Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `Nebula Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a Nebula Go.
:::
...
Nebula Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua Nebula config
utilizza il formato `nebula-go/<model-id>`. Ad esempio, per Kimi K3, useresti
`nebula-go/kimi-k3` nella tua configurazione.
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con Nebula.
