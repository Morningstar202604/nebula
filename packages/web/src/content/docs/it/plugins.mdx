---
title: Plugin
description: Scrivi plugin per estendere Nebula.
---

I plugin ti permettono di estendere Nebula agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Nebula.
...
- `.nebula/plugins/` - plugin a livello progetto
- `~/.config/nebula/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/nebula/node_modules/`.
...
1. Config globale (`~/.config/nebula/nebula.json`)
2. Config di progetto (`nebula.json`)
3. Directory plugin globale (`~/.config/nebula/plugins/`)
4. Directory plugin di progetto (`.nebula/plugins/`)
...
Nebula esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di nebula per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere nebula.
...
:::note
Se usi l'app desktop di Nebula, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a nebula di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a nebula:
...
L'helper `tool` crea uno strumento personalizzato che nebula può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in nebula insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://nebula.ai/docs/sdk) per i dettagli.
