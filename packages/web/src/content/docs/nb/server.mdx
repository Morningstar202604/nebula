---
title: Tjener
description: Samhandle med Nebula-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `nebula serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Nebula-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `nebula.local` |
...
Sett `NEBULA_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `nebula` som standard, eller sett `NEBULA_SERVER_USERNAME` for å overstyre det. Dette gjelder både `nebula serve` og `nebula web`.
...
Når du kjører `nebula` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Nebula-serveren til å samhandle med Nebula programmatisk.
:::

Denne arkitekturen lar Nebula støtte flere klienter og lar deg samhandle med Nebula programmatisk.

Du kan kjøre `nebula serve` for å starte en frittstående server. Hvis du har
Nebula TUI kjører, vil `nebula serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Nebula [IDE](/docs/ide) plugins.
...
nebula-serveren viser følgende APIer.
