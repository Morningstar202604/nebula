...
Du kan legge til eksterne verktøy i Nebula ved å bruke _Model Context Protocol_ eller MCP. Nebula støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Nebula Config](https://nebula.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="nebula.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/nebula`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Nebula håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Nebula:
...
Hvis serveren krever autentisering, vil Nebula be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `nebula mcp auth <server-name>`.
...
```bash
nebula mcp auth my-oauth-server
```
...
```bash
nebula mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Nebula lagre tokens sikkert i `~/.local/share/nebula/mcp-auth.json`.
...
nebula mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Nebula, sammen med innebygde verktøy. Så du kan administrere dem gjennom Nebula-konfigurasjonen som alle andre verktøy.
...
```json title="nebula.json" {11, 14-18}
{
  "$schema": "https://nebula.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
nebula mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Nebula til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Nebula-samtale oppført som en separat økt.
