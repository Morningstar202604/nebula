...
description: Skriv dine egne programtillegg for å utvide Nebula.
---

Plugins lar deg utvide Nebula ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Nebula.
...
- `.nebula/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/nebula/node_modules/`.
...
1. Global konfigurasjon (`~/.config/nebula/nebula.json`)
2. Prosjektkonfigurasjon (`nebula.json`)
3. Global plugin-katalog (`~/.config/nebula/plugins/`)
4. Prosjektpluginkatalog (`.nebula/plugins/`)
...
Nebula kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Nebula SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Nebula.
...
:::note
Hvis du bruker Nebula-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Nebula fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Nebula:
...
`tool`-hjelperen lager et tilpasset verktøy som Nebula kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Nebula sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://nebula.ai/docs/sdk) for detaljer.
