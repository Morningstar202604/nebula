---
title: TUI
description: Using the Nebula terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Nebula provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Nebula starts the TUI for the current directory.
...
When using the Nebula TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Nebula. Allows you to select from available providers and add their API keys.
...
This is separate from `nebula.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long Nebula waits after the leader key. Defaults to `2000`.
...
Use `NEBULA_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `nebula.default`.
