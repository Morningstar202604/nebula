---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Nebula zu erweitern.
---

Mit Plugins können Sie Nebula erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Nebula zu ändern.
...
- `.nebula/plugins/` – Plugins auf Projektebene
- `~/.config/nebula/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/nebula/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/nebula/nebula.json`)
2. Projektkonfiguration (`nebula.json`)
3. Globales Plugin-Verzeichnis (`~/.config/nebula/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.nebula/plugins/`)
...
Nebula führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Nebula-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Nebula erweitern können.
...
:::note
Wenn Sie die Nebula-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Nebula `.env`-Dateien liest:
...
Plugins können Nebula auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Nebula aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Nebula zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://nebula.ai/docs/sdk).
