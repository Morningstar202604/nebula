...
Mit Nebula kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Nebula das `nebula`-Theme.
...
Nebula liefert mehrere eingebaute Themes mit.
...
- Nebula optisch ans Terminal anpassen wollen
...
Nebula unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/nebula/themes/*.json` oder `$XDG_CONFIG_HOME/nebula/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.nebula/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.nebula/themes/*.json`
