...
Nebula Go ist ein kostengünstiges Abonnement für **10 $/Monat**, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in Nebula. Du abonnierst Nebula Go und
...
Nebula Go bietet dir Zugriff auf diese Modelle für **10 $/Monat**.

---

## Wie es funktioniert

Nebula Go funktioniert wie jeder andere Provider in Nebula.

1. Du meldest dich bei **<a href={console}>Nebula Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `Nebula Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann Nebula Go abonnieren.
:::
...
Nebula Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner Nebula Config verwendet das Format `nebula-go/<model-id>`. Für Kimi K3 würdest du beispielsweise `nebula-go/kimi-k3` in deiner Config verwenden.
...
Wir haben Nebula Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit Nebula zu nutzen.
