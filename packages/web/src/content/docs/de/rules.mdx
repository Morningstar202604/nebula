---
title: Regeln
description: Lege eigene Anweisungen fuer nebula fest.
---

Du kannst nebula ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in nebula den Befehl `/init` aus.
...
So kann nebula sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht nebula sie zu erweitern.
...
nebula kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/nebula/AGENTS.md` speichern.
Diese gelten in allen nebula-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Nebula dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/nebula/AGENTS.md` existiert)
...
Beim Start sucht nebula in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/nebula/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `nebula.json` oder in `~/.config/nebula/nebula.json` angeben.
...
nebula parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit nebula.json

Empfohlen ist das `instructions`-Feld in der `nebula.json`:
...
Du kannst nebula explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `nebula.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
