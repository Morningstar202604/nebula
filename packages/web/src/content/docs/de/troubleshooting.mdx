...
Wenn Nebula Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `nebula --log-level DEBUG`.

---

## Speicher

nebula speichert Sitzungs- und App-Daten auf der Festplatte unter:
...
Nebula Desktop startet im Hintergrund einen lokalen Nebula-Server (`nebula-cli`-Sidecar).
...
- Nur macOS: `Nebula`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/nebula/nebula.jsonc` (oder `~/.config/nebula/nebula.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/nebula/nebula.jsonc`
...
Nebula kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Nebula ihn neu aufbauen kann.

1. Beende Nebula Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/nebula`
- **Linux**: loesche `~/.cache/nebula` (oder fuehre aus: `rm -rf ~/.cache/nebula`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\nebula`

3. Starte Nebula Desktop neu.

---

### Server-Verbindungsprobleme beheben

Nebula Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `nebula.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `NEBULA_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `NEBULA_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Nebula Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Nebula-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Nebula Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Nebula in den OS-Einstellungen aktiviert sind, und
...
1. Beende Nebula Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Nebula Desktop):

- `nebula.settings.dat` (desktop default server URL)
- `nebula.global.dat` und `nebula.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Nebula hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `nebula models` aus.
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. nebula installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
2. Starte nebula neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt nebula, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
nebula erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
