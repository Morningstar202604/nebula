...
Agent Skills erlauben Nebula, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Nebula sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Nebula vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.nebula/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/nebula/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.nebula/skills/git-release/SKILL.md` zum Beispiel so:
...
Nebula listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `nebula.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `nebula.json`):
