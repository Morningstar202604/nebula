---
title: Servidor
description: Interactuar con el servidor nebula a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `nebula serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente nebula puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `nebula.local` |
...
Configure `NEBULA_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `nebula`, o configure `NEBULA_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `nebula serve` como a `nebula web`.
...
Cuando ejecuta `nebula`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor nebula para interactuar con nebula mediante programación.
:::

Esta arquitectura permite que nebula admita múltiples clientes y le permite interactuar con nebula mediante programación.

Puede ejecutar `nebula serve` para iniciar un servidor independiente. Si tienes el
nebula TUI ejecutándose, `nebula serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Nebula [IDE](/docs/ide).
...
El servidor nebula expone las siguientes API.
