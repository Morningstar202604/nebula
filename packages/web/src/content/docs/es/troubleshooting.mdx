...
Para depurar problemas con Nebula, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/nebula/log/`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `nebula --log-level DEBUG`.

---

## Almacenamiento

nebula almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/nebula/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\nebula`
...
Nebula Desktop ejecuta un servidor Nebula local (el sidecar `nebula-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Nebula` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/nebula/nebula.jsonc` (o `~/.config/nebula/nebula.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/nebula/nebula.jsonc`
...
Nebula también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/nebula/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\nebula\plugins`
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.nebula/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Nebula pueda reconstruirlo.

1. Salga de Nebula Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/nebula`
- **Linux**: eliminar `~/.cache/nebula` (o ejecutar `rm -rf ~/.cache/nebula`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\nebula`

3. Reinicie el escritorio Nebula.

---

### Solucionar problemas de conexión del servidor

Nebula Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `nebula.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `NEBULA_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `NEBULA_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio Nebula requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Nebula.

---

### Notificaciones que no se muestran

Nebula Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Nebula en la configuración de su sistema operativo, y
...
1. Salga del escritorio Nebula.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Nebula):

- `nebula.settings.dat` (URL del servidor predeterminado de escritorio)
- `nebula.global.dat` y `nebula.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/Morningstar202604/nebula/issues**](https://github.com/Morningstar202604/nebula/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**nebula.ai/discord**](https://nebula.ai/discord)
...
3. Asegúrese de tener la última versión con `nebula upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `nebula models`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. nebula instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
2. Reinicie nebula para reinstalar los paquetes del proveedor más recientes.

Esto obligará a nebula a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
nebula detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
