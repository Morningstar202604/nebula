...
Las herramientas permiten que LLM realice acciones en su código base. Nebula viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Nebula.
...
:::note
Esta herramienta solo está disponible cuando `NEBULA_EXPERIMENTAL_LSP_TOOL=true` (o `NEBULA_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Nebula o cuando la variable de entorno `NEBULA_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Nebula:

```bash
NEBULA_ENABLE_EXA=1 nebula
```
