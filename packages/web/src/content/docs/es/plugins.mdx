---
title: Complementos
description: Escriba sus propios complementos para extender Nebula.
---

Los complementos le permiten extender Nebula al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Nebula.
...
- `.nebula/plugins/` - Complementos a nivel de proyecto
- `~/.config/nebula/plugins/` - Complementos globales
...
```json title="nebula.json"
{
  ...
  "plugin": ["nebula-helicone-session", "nebula-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/nebula/node_modules/`.
...
1. Configuración global (`~/.config/nebula/nebula.json`)
2. Configuración del proyecto (`nebula.json`)
3. Directorio global de complementos (`~/.config/nebula/plugins/`)
4. Directorio de complementos del proyecto (`.nebula/plugins/`)
...
```json title=".nebula/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
Nebula ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".nebula/plugins/my-plugin.ts"
...
```js title=".nebula/plugins/example.js"
...
- `client`: Un cliente SDK nebula para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar nebula.
...
```js title=".nebula/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "nebula"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Nebula, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que nebula lea archivos `.env`:

```javascript title=".nebula/plugins/env-protection.js"
...
```javascript title=".nebula/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a nebula:

```ts title=".nebula/plugins/custom-tools.ts"
import { type Plugin, tool } from "@nebula-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que nebula puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para nebula junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://nebula.ai/docs/sdk) para obtener más detalles.
...
```ts title=".nebula/plugins/compaction.ts"
import type { Plugin } from "@nebula-ai/plugin"
...
```ts title=".nebula/plugins/custom-compaction.ts"
import type { Plugin } from "@nebula-ai/plugin"
