...
Las habilidades del agente permiten a Nebula descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Nebula busca estas ubicaciones:

- Configuración del proyecto: `.nebula/skills/<name>/SKILL.md`
- Configuración global: `~/.config/nebula/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Nebula sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.nebula/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/nebula/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.nebula/skills/git-release/SKILL.md` así:
...
Nebula enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `nebula.json`:
...
**Para agentes integrados** (en `nebula.json`):
