...
Con Nebula puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Nebula usa nuestro propio tema `nebula`.
...
Nebula viene con varios temas integrados.
...
- Quiere que Nebula coincida con la apariencia de su terminal
...
Nebula admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/nebula/themes/*.json` o `$XDG_CONFIG_HOME/nebula/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.nebula/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.nebula/themes/*.json`
