...
description: Cree herramientas que LLM pueda llamar en nebula.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de nebula como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.nebula/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/nebula/tools/`.
...
```ts title=".nebula/tools/database.ts" {1}
...
```ts title=".nebula/tools/project.ts" {8}
import { tool } from "@nebula-ai/plugin"
...
```python title=".nebula/tools/add.py"
import sys
...
```ts title=".nebula/tools/python-add.ts" {10}
import { tool } from "@nebula-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".nebula/tools/add.py")
