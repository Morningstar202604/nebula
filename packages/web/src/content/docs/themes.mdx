...
With Nebula you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Nebula uses our own `nebula` theme.
...
Nebula comes with several built-in themes.
...
- Want Nebula to match their terminal's appearance
...
Nebula supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/nebula/themes/*.json` or `$XDG_CONFIG_HOME/nebula/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.nebula/themes/*.json`
4. **Current working directory** - Defined in `./.nebula/themes/*.json`
