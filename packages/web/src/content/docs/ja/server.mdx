---
title: サーバー
description: HTTP 経由で Nebula サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`nebula serve` コマンドは、Nebula クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `nebula.local` |
...
HTTP 基本認証でサーバーを保護するには、`NEBULA_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `nebula` になるか、`NEBULA_SERVER_USERNAME` を設定してオーバーライドします。これは、`nebula serve` と `nebula web` の両方に当てはまります。
...
`nebula` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Nebula サーバーを使用して、プログラムで Nebula と対話します。
:::

`nebula serve` を実行してスタンドアロンサーバーを起動できます。すでに Nebula TUI を実行している場合でも、`nebula serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Nebula [IDE](/docs/ide) プラグインによって使用されます。
