---
title: Server
description: Interact with nebula server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `nebula serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an nebula client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `nebula.local` |
...
Set `NEBULA_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `nebula`, or set `NEBULA_SERVER_USERNAME` to override it. This applies to both `nebula serve` and `nebula web`.
...
When you run `nebula` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the nebula server to interact with nebula programmatically.
:::

This architecture lets nebula support multiple clients and allows you to interact with nebula programmatically.

You can run `nebula serve` to start a standalone server. If you have the
nebula TUI running, `nebula serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Nebula [IDE](/docs/ide) plugins.
...
The nebula server exposes the following APIs.
