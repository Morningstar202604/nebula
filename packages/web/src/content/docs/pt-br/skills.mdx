...
As habilidades do agente permitem que o nebula descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O nebula pesquisa nesses locais:

- Configuração do projeto: `.nebula/skills/<name>/SKILL.md`
- Configuração global: `~/.config/nebula/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o nebula sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.nebula/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/nebula/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.nebula/skills/git-release/SKILL.md` assim:
...
O nebula lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `nebula.json`:
...
**Para agentes embutidos** (em `nebula.json`):
