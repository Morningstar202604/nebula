---
title: Plugins
description: Escreva seus próprios plugins para estender o nebula.
---

Plugins permitem que você estenda o nebula conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do nebula.
...
- `.nebula/plugins/` - Plugins em nível de projeto
- `~/.config/nebula/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/nebula/node_modules/`.
...
1. Configuração global (`~/.config/nebula/nebula.json`)
2. Configuração do projeto (`nebula.json`)
3. Diretório de plugins global (`~/.config/nebula/plugins/`)
4. Diretório de plugins do projeto (`.nebula/plugins/`)
...
O nebula executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do nebula para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o nebula.
...
:::note
Se você estiver usando o aplicativo desktop nebula, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o nebula de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao nebula:
...
O helper `tool` cria uma ferramenta personalizada que o nebula pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o nebula junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://nebula.ai/docs/sdk) para detalhes.
