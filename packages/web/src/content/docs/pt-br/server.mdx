---
title: Servidor
description: Interaja com o servidor nebula via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `nebula serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente nebula pode usar.
...
```bash
nebula serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `nebula.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
nebula serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `NEBULA_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `nebula`, ou defina `NEBULA_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `nebula serve` quanto ao `nebula web`.

```bash
NEBULA_SERVER_PASSWORD=your-password nebula serve
```
...
Quando você executa `nebula`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor nebula para interagir com o nebula programaticamente.
:::

Essa arquitetura permite que o nebula suporte múltiplos clientes e permite que você interaja com o nebula programaticamente.

Você pode executar `nebula serve` para iniciar um servidor autônomo. Se você tiver o TUI do nebula em execução, `nebula serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do nebula [IDE](/docs/ide).
...
O servidor nebula expõe as seguintes APIs.
