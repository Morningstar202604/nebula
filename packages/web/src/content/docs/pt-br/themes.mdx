...
Com o nebula, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o nebula usa nosso próprio tema `nebula`.
...
O nebula vem com vários temas embutidos.
...
- Querem que o nebula corresponda à aparência do seu terminal
...
O nebula suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/nebula/themes/*.json` ou `$XDG_CONFIG_HOME/nebula/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.nebula/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.nebula/themes/*.json`
