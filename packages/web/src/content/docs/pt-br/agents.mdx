...
Existem dois tipos de agentes no nebula; agentes primários e subagentes.
...
nebula vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Nebula vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

Nebula vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do Nebula, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `nebula.json`:
...
- Global: `~/.config/nebula/agents/`
- Por projeto: `.nebula/agents/`

```markdown title="~/.config/nebula/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o nebula usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do nebula quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do nebula usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Nebula Zen](/docs/zen), você usaria `nebula/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="nebula.json" {3-6,9-12}
{
  "$schema": "https://nebula.ai/config.json",
  "tools": {
    "write": true,
...
```json title="nebula.json" {3-5,8-10}
{
  "$schema": "https://nebula.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `nebula models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/Morningstar202604/nebula).
:::
...
```markdown title="~/.config/nebula/agents/docs-writer.md"
---
...
```markdown title="~/.config/nebula/agents/security-auditor.md"
---
