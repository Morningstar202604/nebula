...
Para depurar problemas com o nebula, comece verificando os logs e os dados locais que ele armazena no disco.
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `nebula --log-level DEBUG`.

---

## Armazenamento

O nebula armazena dados de sessão e outros dados do aplicativo no disco em:
...
O nebula Desktop executa um servidor nebula local (o sidecar `nebula-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `Nebula` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux**: `~/.config/nebula/nebula.jsonc` (ou `~/.config/nebula/nebula.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/nebula/nebula.jsonc`
...
O nebula também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o nebula possa reconstruí-lo.

1. Saia completamente do nebula Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/nebula`
- **Linux**: exclua `~/.cache/nebula` (ou execute `rm -rf ~/.cache/nebula`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\nebula`

3. Reinicie o nebula Desktop.

---

### Corrigindo problemas de conexão com o servidor

O nebula Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `nebula.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `NEBULA_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `NEBULA_PORT` (ou escolha uma porta livre) e reinicie.
...
No Windows, o nebula Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do nebula.

---

### Notificações não aparecendo

O nebula Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o nebula nas configurações do seu sistema operacional, e
...
1. Saia do nebula Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo nebula Desktop):

- `nebula.settings.dat` (URL do servidor padrão do desktop)
- `nebula.global.dat` e `nebula.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o nebula:
...
3. Certifique-se de que você tem a versão mais recente com `nebula upgrade`
...
Para descobrir quais modelos você tem acesso, execute `nebula models`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O nebula instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
2. Reinicie o nebula para reinstalar os pacotes de provedores mais recentes

Isso forçará o nebula a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O nebula detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
