---
title: Config
description: Using the Nebula JSON config.
---

You can configure Nebula using a JSON config file.

---

## Format

Nebula supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="nebula.jsonc"
{
...
1. **Remote config** (from `.well-known/nebula`) - organizational defaults
2. **Global config** (`~/.config/nebula/nebula.json`) - user preferences
3. **Custom config** (`NEBULA_CONFIG` env var) - custom overrides
4. **Project config** (`nebula.json` in project) - project-specific settings
5. **`.nebula` directories** - agents, commands, plugins
6. **Inline config** (`NEBULA_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/nebula/` on macOS) - admin-controlled
...
:::note
The `.nebula` and `~/.config/nebula` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/nebula` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/nebula"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global Nebula config in `~/.config/nebula/nebula.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/nebula/tui.json`.
...
Add `nebula.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When Nebula starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `NEBULA_CONFIG` environment variable.

```bash
export NEBULA_CONFIG=/path/to/my/custom-config.json
nebula run "Hello world"
```
...
Specify a custom config directory using the `NEBULA_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.nebula` directory, and should
follow the same structure.

```bash
export NEBULA_CONFIG_DIR=/path/to/my/config-directory
nebula run "Hello world"
```

The custom directory is loaded after the global config and `.nebula` directories, so it **can override** their settings.
...
Drop an `nebula.json` or `nebula.jsonc` file in the system managed config directory:
...
On macOS, Nebula reads managed preferences from the `ai.nebula.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

Nebula checks these paths:

1. `/Library/Managed Preferences/<user>/ai.nebula.managed.plist`
2. `/Library/Managed Preferences/ai.nebula.managed.plist`

The plist keys map directly to `nebula.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.nebula.managed` PayloadType. The Nebula config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.nebula.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.nebula.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.nebula</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`nebula.ai/config.json`**](https://nebula.ai/config.json).

TUI config uses [**`nebula.ai/tui.json`**](https://nebula.ai/tui.json).
...
Use `NEBULA_TUI_CONFIG` to point to a custom TUI config file.
...
Legacy `theme`, `keybinds`, and `tui` keys in `nebula.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `nebula serve` and `nebula web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your Nebula server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `nebula.local`. Useful for running multiple instances on the same network.
...
If not specified, Nebula will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your Nebula config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, Nebula tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny Nebula actions on configured resources. Currently, policies can control which providers Nebula may use.
...
Nebula normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, Nebula omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/nebula/agents/` or `.nebula/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Nebula will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`nebula run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/nebula/commands/` or `.nebula/commands/`. [Learn more here](/docs/commands).
...
Nebula uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
Nebula will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, nebula **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend Nebula with custom tools, hooks, and integrations.

Place plugin files in `.nebula/plugins/` or `~/.config/nebula/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="nebula.json"
{
  "$schema": "https://nebula.ai/config.json",
  "plugin": ["nebula-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict Nebula to only use specific providers rather than disabling them one by one.
