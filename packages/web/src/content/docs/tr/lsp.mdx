...
description: Nebula, LSP sunucularınızla bütünleşir.
---

Nebula, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

Nebula, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`NEBULA_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve nebula bir dosyayı açtığında:
...
LSP sunucularını, nebula yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
