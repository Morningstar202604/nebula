---
title: Eklentiler
description: Nebula'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Nebula'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Nebula'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.nebula/plugins/` - Proje düzeyinde eklentiler
- `~/.config/nebula/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/nebula/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/nebula/nebula.json`)
2. Project config (`nebula.json`)
3. Global eklenti dizini (`~/.config/nebula/plugins/`)
4. Proje eklenti dizini (`.nebula/plugins/`)
...
Nebula bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Nebula SDK istemcisi.
...
Nebula'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Nebula masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Nebula'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Nebula'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Nebula'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Nebula'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://nebula.ai/docs/sdk) bakın.
