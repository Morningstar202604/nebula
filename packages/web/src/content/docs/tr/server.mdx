---
title: Sunucu
description: nebula sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`nebula serve` komutu, nebula istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `nebula.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `NEBULA_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `nebula` değeridir; değiştirmek isterseniz `NEBULA_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `nebula serve` hem de `nebula web` için geçerlidir.
...
`nebula` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
nebula ile programatik etkileşim için nebula sunucusunu kullanın.
:::

Bu mimari, nebula'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`nebula serve` ile bağımsız bir sunucu başlatabilirsiniz. nebula TUI açıksa, `nebula serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum nebula [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
nebula sunucusu aşağıdaki API'leri sunar.
