...
_Model Bağlam Protokolü_ veya MCP kullanarak nebula'a harici araçlar ekleyebilirsiniz. nebula hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [nebula Config](https://nebula.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/nebula` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
nebula, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde nebula şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa nebula, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `nebula mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından nebula, belirteçleri `~/.local/share/nebula/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra nebula'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi nebula yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve nebula'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
