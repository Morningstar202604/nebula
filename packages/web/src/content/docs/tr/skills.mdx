...
Ajan becerileri, nebula'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
nebula su konumlari tarar:

- Proje konfigurasyonu: `.nebula/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/nebula/skills/<name>/SKILL.md`
...
Proje yerel yollarinda nebula, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.nebula/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/nebula/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.nebula/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
nebula, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`nebula.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`nebula.json` icinde):
