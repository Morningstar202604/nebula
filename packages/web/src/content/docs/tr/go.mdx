...
Nebula Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli **aylık 10$ aboneliğidir**.

Go, Nebula'daki diğer sağlayıcılar gibi çalışır. Nebula Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Nebula'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Nebula Go, bu modellere **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Nebula Go, Nebula'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Nebula Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Nebula Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Nebula Go'ya abone olabilir.
:::
...
Nebula Go aşağıdaki limitleri içerir:
...
Nebula yapılandırmanızdaki [model id](/docs/config/#models) formatı `nebula-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `nebula-go/kimi-k3` kullanmalısınız.
...
Nebula Go'yu şu amaçlarla oluşturduk:
...
4. Nebula ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
