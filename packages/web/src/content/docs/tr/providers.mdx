---
title: Sağlayıcılar
description: nebula'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

nebula, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı nebula yapılandırmanızda yapılandırın.
...
`~/.local/share/nebula/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları nebula'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Nebula Zen, nebula ekibi tarafından sağlanan modellerin bir listesidir.
nebula ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Nebula Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, nebula'u seçin ve [nebula.ai/auth](https://nebula.ai/auth)'ye gidin.
...
nebula'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Nebula Go

Nebula Go, nebula ile iyi çalıştığı test edilmiş ve doğrulanmış, nebula ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Nebula Go`'yu seçin ve [nebula.ai/auth](https://nebula.ai/zen) adresine gidin.
...
Amazon Bedrock'u nebula ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   nebula'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `nebula.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi nebula'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
nebula'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   nebula'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Nebula yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
nebula, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. nebula'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`nebula.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`nebula.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi nebula'la kullanmak için:
...
Google Vertex AI'yi nebula ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları nebula çalıştırılırken ayarlayın.
...
Helicone'un nebula aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her nebula görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [nebula-helicone-session](https://github.com/H2Shami/nebula-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her nebula görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
nebula'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla nebula'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla nebula'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini nebula için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/nebula)'a bakın.
:::
...
Ollama Cloud'u nebula ile kullanmak için:
...
3. nebula'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: nebula'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Nebula Zen, nebula ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Nebula Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Nebula Zen**'i arayın.

   ```txt
   /connect
   ```

3. nebula API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   nebula yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca nebula yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Nebula ile kullanmak için:
...
Modelleri nebula yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı nebula ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `nebula.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları nebula'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `nebula auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için nebula yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, nebula yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
