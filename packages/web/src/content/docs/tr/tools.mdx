...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. nebula yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
nebula içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `NEBULA_EXPERIMENTAL_LSP_TOOL=true` (veya `NEBULA_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca nebula provider kullanılırken veya `NEBULA_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

nebula başlatırken etkinleştirmek için:
