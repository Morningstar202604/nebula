...
nebula ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\nebula\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `nebula --log-level DEBUG`.

---

## Depolama

nebula, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:
...
nebula Desktop arka planda yerel bir nebula sunucusu (`nebula-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `nebula` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/nebula/nebula.jsonc` (veya `~/.config/nebula/nebula.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/nebula/nebula.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\nebula\nebula.jsonc` yapistirin
...
nebula diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/nebula/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\nebula\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), nebula'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. nebula Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/nebula`
- **Linux**: `~/.cache/nebula` dizinini silin (veya `rm -rf ~/.cache/nebula` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\nebula` yapistirin

3. nebula Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

nebula Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`nebula.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `NEBULA_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `NEBULA_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta nebula Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, nebula ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

nebula Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda nebula icin bildirimler etkinse
...
1. nebula Desktop'u kapatin
2. Su dosyalari bulun ve silin (nebula Desktop uygulama veri dizininde yer alirlar):

- `nebula.settings.dat` (desktop varsayilan sunucu URL'si)
- `nebula.global.dat` ve `nebula.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
nebula ile ilgili bir sorun yasiyorsaniz:
...
3. `nebula upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `nebula models` calistirin.
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. nebula, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
2. En guncel provider paketlerini yeniden kurmak icin nebula'u yeniden baslatin

Bu, nebula'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
nebula, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
