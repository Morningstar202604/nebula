...
nebula'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
nebula, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
Nebula, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

Nebula iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `nebula.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse nebula modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global nebula yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
nebula yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [Nebula Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `nebula/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `nebula models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/Morningstar202604/nebula).
:::
