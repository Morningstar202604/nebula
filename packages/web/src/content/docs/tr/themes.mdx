...
nebula ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak nebula kendi `nebula` temasını kullanır.
...
nebula birden fazla yerleşik temayla gelir.
...
- nebula'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
nebula, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/nebula/themes/*.json` veya `$XDG_CONFIG_HOME/nebula/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.nebula/themes/*.json`
4. **Geçerli çalışma dizini** - `./.nebula/themes/*.json`
