...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Nebula posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Nebula.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `NEBULA_EXPERIMENTAL_LSP_TOOL=true` (lub `NEBULA_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Nebula lub gdy zmienna środowiskowa `NEBULA_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Nebula:
