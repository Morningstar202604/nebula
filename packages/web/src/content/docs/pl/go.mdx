...
Nebula Go to niskokosztowa subskrypcja za **10 $/miesiąc**, która zapewnia niezawodny dostęp do popularnych otwartych modeli do kodowania.

Go działa jak każdy inny dostawca w Nebula. Subskrybujesz Nebula Go i
...
Nebula Go daje Ci dostęp do tych modeli za **10 $/miesiąc**.

---

## Jak to działa

Nebula Go działa jak każdy inny dostawca w Nebula.

1. Logujesz się do **<a href={console}>Nebula Zen</a>**, subskrybujesz Go i
   kopiujesz swój klucz API.
2. Uruchamiasz komendę `/connect` w TUI, wybierasz `Nebula Go` i wklejasz
   swój klucz API.
3. Uruchom `/models` w TUI, aby zobaczyć listę modeli dostępnych w ramach Go.

:::note
Tylko jeden członek na obszar roboczy (workspace) może zasubskrybować Nebula Go.
:::
...
Nebula Go zawiera następujące limity:
...
[ID modelu](/docs/config/#models) w Twojej konfiguracji Nebula
używa formatu `nebula-go/<model-id>`. Na przykład dla Kimi K3 należy użyć
`nebula-go/kimi-k3` w swojej konfiguracji.
...
4. Zapewnić **brak ograniczeń (no lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy wraz z Nebula.
