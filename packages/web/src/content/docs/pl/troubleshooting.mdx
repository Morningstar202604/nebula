...
Aby debugować problemy z nebula, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `nebula --log-level DEBUG`.

---

## Przechowywanie danych

nebula przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:
...
nebula Desktop uruchamia w tle lokalny serwer nebula (sidecar `nebula-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Nebula` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/nebula/nebula.jsonc` (lub `~/.config/nebula/nebula.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/nebula/nebula.jsonc`
...
nebula może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby nebula mógł ją odbudować.

1. Całkowicie zamknij nebula Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/nebula`
- **Linux**: usuń `~/.cache/nebula` (lub uruchom `rm -rf ~/.cache/nebula`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\nebula`

3. Uruchom ponownie nebula Desktop.

---

### Napraw problemy z połączeniem z serwerem

nebula Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `nebula.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `NEBULA_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `NEBULA_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows nebula Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami nebula.

---

### Brak powiadomień

nebula Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla nebula w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie nebula Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji nebula Desktop):

- `nebula.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `nebula.global.dat` i `nebula.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `nebula upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `nebula models`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. nebula dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
2. Uruchom ponownie kod nebula, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to nebula do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
nebula wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
