---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w nebula.
---

import config from "../../../../config.mjs"
export const console = config.console

nebula używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji nebula.
...
w `~/.local/share/nebula/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim nebula
config.
...
Nebula Zen to lista modeli dostarczonych przez zespół nebula, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z nebula. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Nebula Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Nebula Zen` i przejdź do [nebula.ai/auth](https://nebula.ai/zen).
...
Działa jak każdy inny dostawca w nebula i jest całkowicie opcjonalny w użyciu.

---

## Nebula Go

Nebula Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół nebula, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z nebula.

1. Uruchom polecenie `/connect` w TUI, wybierz `Nebula Go` i przejdź do [nebula.ai/auth](https://nebula.ai/zen).
...
Aby używać Amazon Bedrock z nebula:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania nebula:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `nebula.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w nebula nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować nebula do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod nebula działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji nebula.
...
nebula używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować nebula
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`nebula.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem nebula:
...
Aby używać Google Vertex AI z nebula:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania nebula.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą nebula, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [nebula-helicone-session](https://github.com/H2Shami/nebula-helicone-session), aby automatycznie rejestrować każdą konwersację nebula jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja nebula będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować nebula tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować nebula tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować nebula tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem nebula. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/nebula).
:::
...
Aby korzystać z Ollama Cloud z nebula:
...
3. Skopiuj klucz API do użycia w nebula.
...
6. **Ważne**: Przed użyciem modeli chmurowych w nebula musisz pobrać informacje o modelu lokalnie:
...
Nebula Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół nebula. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Nebula Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Nebula Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API nebula.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji nebula.
...
5. Możesz je także dostosować za pomocą konfiguracji nebula. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z nebula:
...
Możesz także dostosować modele za pomocą konfiguracji nebula. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z nebula. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `nebula.json` w katalogu projektu:
...
Pola `limit` pozwalają nebula zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `nebula auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację nebula i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji nebula.
