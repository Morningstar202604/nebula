...
Możesz dodać dodatkowe informacje do nebula za pomocą _Model Context Protocol_, w skrócie MCP. nebula obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [nebula Config](https://nebula.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/nebula`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
nebula automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, nebula:
...
Jeśli serwer wymaga uwierzytelnienia, nebula poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `nebula mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji nebula bezpieczne przechowa tokeny w `~/.local/share/nebula/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w nebula, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie nebula, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć nebula z kontem Sentry.
