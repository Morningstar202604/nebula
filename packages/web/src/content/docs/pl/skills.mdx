...
Umiejętności agenta pozwalają nebula odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
nebula przeszukuje te lokalizacje:

- Project config: `.nebula/skills/<name>/SKILL.md`
- Global config: `~/.config/nebula/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu nebula przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.nebula/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/nebula/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.nebula/skills/git-release/SKILL.md` w ten sposób:
...
nebula wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `nebula.json`:
...
**Dla agentów wbudowanych** (w `nebula.json`):
