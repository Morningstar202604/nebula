---
title: Serwer
description: Interakcja z serwerem nebula za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `nebula serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient nebula.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `nebula.local` |
...
Ustaw `NEBULA_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `nebula` lub ustaw `NEBULA_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `nebula serve`, jak i `nebula web`.
...
Kiedy uruchomisz `nebula`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera nebula do programowej interakcji z nebula.
:::

Ta architektura umożliwia obsługę wielu klientów przez nebula i programową interakcję z kodem otwartym.

Możesz uruchomić `nebula serve`, aby uruchomić samodzielny serwer. Jeśli masz
nebula TUI działa, `nebula serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki nebula [IDE](/docs/ide).
...
Serwer nebula udostępnia następujące interfejsy API.
