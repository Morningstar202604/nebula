...
Dzięki nebula możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie nebula używa naszego własnego motywu `nebula`.
...
nebula ma kilka wbudowanych motywów.
...
- Chcą, aby Nebula pasował do wyglądu ich terminala
...
nebula obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/nebula/themes/*.json` lub `$XDG_CONFIG_HOME/nebula/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.nebula/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.nebula/themes/*.json`
