---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć nebula.
---

Wtyczki umożliwiające wyłączenie nebula poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania nebula.
...
- `.nebula/plugins/` - Wtyczki na poziomie projektu
- `~/.config/nebula/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/nebula/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/nebula/nebula.json`)
2. Konfiguracja projektu (`nebula.json`)
3. Globalny katalog wtyczek (`~/.config/nebula/plugins/`)
4. Katalog wtyczek projektu (`.nebula/plugins/`)
...
nebula uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z nebula do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do nebula.
...
:::note
Jeśli korzystasz z aplikacji komputerowej nebula, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj nebula czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do nebula:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod nebula. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://nebula.ai/docs/sdk).
