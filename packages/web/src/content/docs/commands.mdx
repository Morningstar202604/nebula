...
Create `.nebula/commands/test.md`:
...
You can add custom commands through the Nebula config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Nebula [config](/docs/config):
...
nebula includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
