...
تتيح مهارات الوكيل لـ Nebula اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Nebula في هذه المواقع:

- إعدادات المشروع: `.nebula/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/nebula/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Nebula صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.nebula/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/nebula/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.nebula/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Nebula المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `nebula.json`:
...
**للوكلاء المدمجين** (في `nebula.json`):
