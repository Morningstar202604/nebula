...
أنشئ الملف `.nebula/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Nebula أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Nebula:
...
- على مستوى النظام: `~/.config/nebula/commands/`
- لكل مشروع: `.nebula/commands/`
...
يتضمن nebula عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
