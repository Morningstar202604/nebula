...
مع Nebula يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Nebula سمتنا `nebula`.
...
- يريدون أن يطابق Nebula مظهر terminal لديهم
...
يدعم Nebula نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/nebula/themes/*.json` أو `$XDG_CONFIG_HOME/nebula/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.nebula/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.nebula/themes/*.json`
