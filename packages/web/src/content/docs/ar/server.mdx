---
title: الخادم
description: تفاعل مع خادم nebula عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `nebula serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل nebula استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `nebula.local` |
...
عيّن `NEBULA_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `nebula`، أو عيّن `NEBULA_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `nebula serve` و `nebula web`.
...
عند تشغيل `nebula` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم nebula للتفاعل مع nebula برمجيا.
:::

تتيح هذه البنية لـ nebula دعم عدة عملاء وتمكّنك من التفاعل مع nebula برمجيا.

يمكنك تشغيل `nebula serve` لبدء خادم مستقل. إذا كانت واجهة nebula في terminal (TUI)
قيد التشغيل، فسيبدأ `nebula serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Nebula لـ [IDE](/docs/ide).
...
يعرض خادم nebula واجهات API التالية.
