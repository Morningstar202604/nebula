---
title: المزوّدون
description: استخدام أي موفّر LLM في Nebula.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Nebula ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Nebula.
...
في `~/.local/share/nebula/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Nebula.
...
Nebula Zen هي قائمة نماذج يوفّرها فريق Nebula وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Nebula. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Nebula Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر nebula، ثم انتقل إلى [nebula.ai/auth](https://nebula.ai/auth).
...
يعمل مثل أي مزوّد آخر في Nebula واستخدامه اختياري بالكامل.

---

## Nebula Go

Nebula Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Nebula، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Nebula.

1. شغّل الأمر `/connect` في TUI، واختر `Nebula Go`، ثم انتقل إلى [nebula.ai/auth](https://nebula.ai/zen).
...
لاستخدام Amazon Bedrock مع Nebula:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل nebula:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `nebula.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Nebula غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين nebula لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل nebula بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات nebula.
...
تخزين الرمز في مساحة تخزين مصادقة nebula.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Nebula نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Nebula
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`nebula.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع nebula:
...
لاستخدام Google Vertex AI مع Nebula:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر nebula، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [nebula-helicone-session](https://github.com/H2Shami/nebula-helicone-session) لتسجيل كل محادثة Nebula تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Nebula مدرجة كجلسة مستقلة.
...
يمكنك ضبط nebula لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط nebula لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط nebula لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Nebula. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/nebula) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Nebula:
...
3. انسخ مفتاح API لاستخدامه في Nebula.
...
6. **مهم**: قبل استخدام نماذج السحابة في Nebula، يجب سحب معلومات النموذج محليا:
...
Nebula Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Nebula. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Nebula Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Nebula Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Nebula.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات nebula.
...
5. يمكنك أيضا تخصيصها عبر إعدادات nebula. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Nebula:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات nebula. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع nebula. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `nebula.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Nebula بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `nebula auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات nebula وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات nebula.
