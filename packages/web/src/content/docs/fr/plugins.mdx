---
title: Plugins
description: Écrivez vos propres plugins pour étendre Nebula.
---

Les plugins vous permettent d'étendre Nebula en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Nebula.
...
- `.nebula/plugins/` - Plugins au niveau du projet
- `~/.config/nebula/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/nebula/node_modules/`.
...
1. Configuration globale (`~/.config/nebula/nebula.json`)
2. Configuration du projet (`nebula.json`)
3. Répertoire global des plugins (`~/.config/nebula/plugins/`)
4. Répertoire des plugins du projet (`.nebula/plugins/`)
...
Nebula exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK nebula pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre nebula.
...
:::note
Si vous utilisez l'application de bureau Nebula, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez nebula de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à nebula :
...
L'assistant `tool` crée un outil personnalisé que nebula peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour nebula aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://nebula.ai/docs/sdk) pour plus de détails.
