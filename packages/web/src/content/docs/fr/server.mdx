---
title: Serveur
description: Interagissez avec le serveur nebula via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `nebula serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client nebula peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `nebula.local` |
...
Définissez `NEBULA_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `nebula`, ou définissez `NEBULA_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `nebula serve` et à `nebula web`.
...
Lorsque vous exécutez `nebula`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur nebula pour interagir avec nebula par programmation.
:::

Cette architecture permet à nebula de prendre en charge plusieurs clients et vous permet d'interagir avec nebula par programme.

Vous pouvez exécuter `nebula serve` pour démarrer un serveur autonome. Si vous avez le
nebula TUI en cours d'exécution, `nebula serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Nebula [IDE](/docs/ide).
...
Le serveur nebula expose les API suivantes.
