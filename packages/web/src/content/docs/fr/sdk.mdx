---
title: SDK
description: Client JS de type sécurisé pour le serveur nebula.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK nebula JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler nebula par programme.
...
Créez une instance de nebula :

```javascript
import { createNebula } from "@nebula-ai/sdk"

const { client } = await createNebula()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `nebula.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${nebula.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de nebula, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@nebula-ai/sdk"
```
