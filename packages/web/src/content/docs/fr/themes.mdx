...
Avec Nebula, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Nebula utilise notre propre thème `nebula`.
...
Nebula est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Nebula corresponde à l'apparence de leur terminal
...
Nebula prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/nebula/themes/*.json` ou `$XDG_CONFIG_HOME/nebula/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.nebula/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.nebula/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/nebula/themes
vim ~/.config/nebula/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .nebula/themes
vim .nebula/themes/my-theme.json
```
...
{
  "$schema": "https://nebula.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
