...
To debug issues with Nebula, start by checking the logs and local data it stores on disk.
...
You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `nebula --log-level DEBUG`.

---

## Storage

nebula stores session data and other application data on disk at:
...
To uninstall the Nebula CLI and remove its related files, run:
...
To remove Nebula Desktop, uninstall the application through your operating system's app management tools.

---

## Desktop app

Nebula Desktop runs a local Nebula server (the `nebula-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `Nebula` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
- **macOS/Linux**: `~/.config/nebula/nebula.jsonc` (or `~/.config/nebula/nebula.json`)
- **macOS/Linux** (older installs): `~/.local/share/nebula/nebula.jsonc`
...
Nebula can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so Nebula can rebuild it.

1. Quit Nebula Desktop completely.
2. Delete the cache directory:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/nebula`
- **Linux**: delete `~/.cache/nebula` (or run `rm -rf ~/.cache/nebula`)
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.cache\nebula`

3. Restart Nebula Desktop.

---

### Fix server connection issues

Nebula Desktop can either start its own local server (default) or connect to a server URL you configured.
...
If your `nebula.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.

#### Check environment variables

If you have `NEBULA_PORT` set in your environment, the desktop app will try to use that port for the local server.

- Unset `NEBULA_PORT` (or pick a free port) and restart.
...
On Windows, Nebula Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with Nebula's features.

---

### Notifications not showing

Nebula Desktop only shows system notifications when:

- notifications are enabled for Nebula in your OS settings, and
...
1. Quit Nebula Desktop.
2. Find and delete these files (they live in the Nebula Desktop app data directory):

- `nebula.settings.dat` (desktop default server URL)
- `nebula.global.dat` and `nebula.workspace.*.dat` (UI state like recent servers/projects)
...
If you're experiencing issues with Nebula:
...
3. Ensure you have the latest version with `nebula upgrade`
...
To figure out what models you have access to, run `nebula models`
...
If you encounter API call errors, this may be due to outdated provider packages. nebula dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
2. Restart nebula to reinstall the latest provider packages

This will force nebula to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
nebula will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
