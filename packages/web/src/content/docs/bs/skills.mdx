...
Agent skills omogucavaju Nebula da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Nebula pretrazuje ove lokacije:

- Konfiguracija projekta: `.nebula/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/nebula/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Nebula ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.nebula/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/nebula/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.nebula/skills/git-release/SKILL.md` ovako:
...
Nebula navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `nebula.json`:
...
**Za ugradene agente** (u `nebula.json`):
