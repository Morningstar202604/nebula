---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Nebula.
---

Dodaci vam omogućavaju da proširite Nebula spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Nebula.
...
- `.nebula/plugins/` - Dodaci na nivou projekta
- `~/.config/nebula/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/nebula/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/nebula/nebula.json`)
2. Konfiguracija projekta (`nebula.json`)
3. Globalni direktorij dodataka (`~/.config/nebula/plugins/`)
4. Direktorij dodataka projekta (`.nebula/plugins/`)
...
Nebula pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Nebula SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Nebula.
...
Spriječite nebula da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u nebula:
...
Pomoćnik `tool` kreira prilagođeni alat koji nebula može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za nebula zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://nebula.ai/docs/sdk) za detalje.
