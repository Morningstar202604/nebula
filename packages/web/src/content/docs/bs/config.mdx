---
title: Konfiguracija
description: Korištenje Nebula JSON konfiguracije.
---

Možete konfigurirati Nebula koristeći JSON konfiguracijski fajl.

---

## Format

Nebula podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/nebula`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/nebula/nebula.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`NEBULA_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`nebula.json` u projektu) - postavke specifične za projekat
5. **`.nebula` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`NEBULA_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.nebula` i `~/.config/nebula` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/nebula` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Nebula konfiguraciju u `~/.config/nebula/nebula.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/nebula/tui.json`.
...
Dodajte `nebula.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Nebula pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `NEBULA_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `NEBULA_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.nebula` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.nebula`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`nebula.ai/config.json`**](https://nebula.ai/config.json).

TUI konfiguracija koristi [**`nebula.ai/tui.json`**](https://nebula.ai/tui.json).
...
Koristite `NEBULA_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `nebula.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `nebula serve` i `nebula web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Nebula server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `nebula.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Nebula konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Nebula pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/nebula/agents/` ili `.nebula/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Nebula će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`nebula run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/nebula/commands/` ili `.nebula/commands/`. [Saznajte više ovdje](/docs/commands).
...
Nebula će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Nebula **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Nebula sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.nebula/plugins/` ili `~/.config/nebula/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Nebula da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
