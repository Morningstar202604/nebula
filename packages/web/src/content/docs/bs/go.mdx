...
Nebula Go je povoljna pretplata od **$10/mjesečno** koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u Nebula-u. Pretplatite se na Nebula Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili Nebula.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa Nebula-om.
:::
...
Nebula Go vam daje pristup ovim modelima za **$10/mjesečno**.

---

## Kako funkcioniše

Nebula Go radi kao bilo koji drugi provajder u Nebula-u.

1. Prijavite se na **<a href={console}>Nebula Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `Nebula Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na Nebula Go.
:::
...
Nebula Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj Nebula konfiguraciji
koristi format `nebula-go/<model-id>`. Na primjer, za Kimi K3, koristili biste
`nebula-go/kimi-k3` u svojoj konfiguraciji.
...
Napravili smo Nebula Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz Nebula koristite i bilo kojeg drugog provajdera.
