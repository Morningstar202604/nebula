---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Nebula.
---

import config from "../../../../config.mjs"
export const console = config.console

Nebula koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Nebula konfiguraciji.
...
u `~/.local/share/nebula/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Nebula
config.
...
Nebula Zen je lista modela koje je obezbedio Nebula tim koji su bili
testirano i potvrđeno da dobro radi sa Nebula. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Nebula Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite nebula i idite na [nebula.ai/auth](https://nebula.ai/auth).
...
Radi kao i svaki drugi provajder u Nebula i potpuno je opcionalan za korištenje.

---

## Nebula Go

Nebula Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Nebula tim i koji su testirani i verificirani da dobro rade s Nebula-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Nebula Go` i idite na [nebula.ai/auth](https://nebula.ai/zen).
...
Da biste koristili Amazon Bedrock s Nebula:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete nebula:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `nebula.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Nebula.
:::
...
Možete konfigurirati nebula za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi nebula ispravno radio.
   :::
...
Također možete dodati modele kroz svoju nebula konfiguraciju.
...
da pohrani token u nebula auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Nebula koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Nebula
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`nebula.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Da biste koristili svoju GitHub Copilot pretplatu s nebula:
...
Za korištenje Google Vertex AI s Nebula:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće nebula.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem nebulaa, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [nebula-helicone-session](https://github.com/H2Shami/nebula-helicone-session) da automatski prijavite svaki Nebula razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Nebula razgovor naveden kao zasebna sesija.
...
Možete konfigurirati nebula za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati nebula za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati nebula za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Nebula. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/nebula) za detalje.
:::
...
Da biste koristili Ollama Cloud s Nebula:
...
3. Kopirajte API ključ za korištenje u Nebula.
...
6. **Važno**: Prije upotrebe modela oblaka u Nebula, morate lokalno povući informacije o modelu:
...
Nebula Zen je lista testiranih i verifikovanih modela koju je obezbedio Nebula tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Nebula Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Nebula Zen**.

```txt
   /connect
```

3. Unesite svoj Nebula API ključ.
...
Također možete dodati dodatne modele putem vaše nebula konfiguracije.
...
5. Također ih možete prilagoditi putem vaše nebula konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Nebulaom:
...
Također možete prilagoditi modele kroz svoju nebula konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s nebula-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `nebula.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Nebula da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `nebula auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Nebula konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj nebula konfiguraciji.
