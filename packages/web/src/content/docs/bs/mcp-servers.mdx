...
Možete dodati vanjske alate u Nebula koristeći _Model Context Protocol_, ili MCP. Nebula podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Nebula Config](https://nebula.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/nebula`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Nebula automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Nebula će:
...
Ako server zahtijeva autentifikaciju, Nebula će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `nebula mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Nebula sigurno čuva tokene u `~/.local/share/nebula/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Nebula, zajedno s ugrađenim alatima. Možete njima upravljati kroz Nebula konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Nebula sa vašim Sentry nalogom.
