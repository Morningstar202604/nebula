...
Da biste otklonili probleme s Nebula, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `nebula --log-level DEBUG`.

---

## Pohrana

Nebula pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:
...
Nebula Desktop pokreće lokalni Nebula server (`nebula-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Nebula` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/nebula/nebula.jsonc` (ili `~/.config/nebula/nebula.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/nebula/nebula.jsonc`
...
Nebula također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Nebula mogao ponovo izgraditi.

1. Potpuno zatvorite Nebula Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/nebula`
- **Linux**: obrišite `~/.cache/nebula` (ili pokrenite `rm -rf ~/.cache/nebula`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\nebula`

3. Ponovo pokrenite Nebula Desktop.

---

### Rješavanje problema sa vezom na serveru

Nebula Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `nebula.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `NEBULA_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `NEBULA_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Nebula Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Nebula karakteristikama.

---

### Obavještenja se ne prikazuju

Nebula Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Nebula u postavkama vašeg OS-a, i
...
1. Zatvorite Nebula Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Nebula Desktop aplikacije):

- `nebula.settings.dat` (URL zadanog servera za desktop)
- `nebula.global.dat` i `nebula.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `nebula models`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Nebula dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
2. Ponovo pokrenite Nebula da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Nebula da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Nebula će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
