---
title: Server
description: Komunicirajte s nebula serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `nebula serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji nebula klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `nebula.local` |
...
Postavite `NEBULA_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `nebula`, ili postavite `NEBULA_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `nebula serve` i za `nebula web`.
...
Kada pokrenete `nebula`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite nebula server za programsku interakciju sa nebula-om.
:::

Ova arhitektura omogucava nebula podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `nebula serve` da startate standalone server. Ako je nebula TUI vec pokrenut, `nebula serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Nebula [IDE](/docs/ide) pluginovi.
...
nebula server izlaže sljedece API-je.
