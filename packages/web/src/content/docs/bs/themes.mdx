...
U Nebula mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Nebula koristi nasu `nebula` temu.
...
Nebula dolazi sa vise ugradenih tema.
...
- Zele da Nebula odgovara izgledu njihovog terminala
...
Nebula podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/nebula/themes/*.json` ili `$XDG_CONFIG_HOME/nebula/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.nebula/themes/*.json`
4. **Trenutni radni direktorij** - `./.nebula/themes/*.json`
