...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Nebula dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Nebula.
...
:::note
Ovaj alat je dostupan samo kada je `NEBULA_EXPERIMENTAL_LSP_TOOL=true` (ili `NEBULA_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Nebula provajdera ili kada je varijabla `NEBULA_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Nebula:
