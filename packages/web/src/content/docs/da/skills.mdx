...
Agentferdigheter lar Nebula oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
Nebula søker etter disse stedene:

- Prosjektkonfigurasjon: `.nebula/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/nebula/skills/<name>/SKILL.md`
...
For prosjektlokale stier går Nebula opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.nebula/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/nebula/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.nebula/skills/git-release/SKILL.md` slik:
...
Nebula viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `nebula.json`:
...
**For innebygde agenter** (i `nebula.json`):
