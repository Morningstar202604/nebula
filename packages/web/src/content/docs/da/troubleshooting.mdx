...
For at fejlfinde problemer med Nebula, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `nebula --log-level DEBUG`.

---

## Lagring

nebula gemmer sessionsdata og andre applikationsdata på disken på:
...
Nebula Desktop kører en lokal Nebula-server (`nebula-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Nebula`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/nebula/nebula.jsonc` (eller `~/.config/nebula/nebula.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/nebula/nebula.jsonc`
...
Nebula kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Nebula kan genopbygge den.

1. Afslut Nebula Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/nebula`
- **Linux**: slet `~/.cache/nebula` (eller kør `rm -rf ~/.cache/nebula`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\nebula`

3. Start Nebula Desktop på ny.

---

### Løs problemer med serverforbindelse

Nebula Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `nebula.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `NEBULA_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `NEBULA_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Nebula Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Nebula.

---

### Meddelelser vises ikke

Nebula Desktop viser kun systemvarsler når:

- varsler er aktiveret for Nebula i dine OS-indstillinger, og
...
1. Afslut Nebula Desktop.
2. Find og slet disse filer (de findes i Nebula Desktop-appens datamappe):

- `nebula.settings.dat` (skrivebordsstandardserver URL)
- `nebula.global.dat` og `nebula.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Nebula:
...
For at finde ud af hvilke modeller du har adgang til, kør `nebula models`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. nebula installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
2. Start nebula på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge nebula til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
nebula vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
