---
title: Plugins
description: Skriv dine egne plugins for at udvide Nebula.
---

Plugins giver dig mulighed for at udvide Nebula ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Nebulas standardadfærd.
...
- `.nebula/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/nebula/node_modules/`.
...
1. Global konfiguration (`~/.config/nebula/nebula.json`)
2. Projektkonfiguration (`nebula.json`)
3. Global plugin-mappe (`~/.config/nebula/plugins/`)
4. Projekt plugin bibliotek (`.nebula/plugins/`)
...
Nebula kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En nebula SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide nebula.
...
:::note
Hvis du bruger Nebula desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå nebula i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til nebula:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som nebula kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for nebula sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://nebula.ai/docs/sdk) for detaljer.
