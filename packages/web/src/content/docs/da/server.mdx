---
title: Server
description: Interager med nebula-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `nebula serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en nebula-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `nebula.local` |
...
Indstil `NEBULA_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `nebula`, eller indstil `NEBULA_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `nebula serve` og `nebula web`.
...
Når du kører `nebula` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug nebula-serveren til at interagere med nebula programmatisk.
:::

Denne arkitektur lader nebula understøtte flere klienter og lader dig interagere med nebula programmatisk.

Du kan køre `nebula serve` for at starte en selvstændig server. Hvis du har
nebula TUI kørende, vil `nebula serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Nebula [IDE](/docs/ide) plugins.
...
Nebula-serveren viser følgende API'er.
