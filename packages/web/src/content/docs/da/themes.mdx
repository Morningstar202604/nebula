...
Med Nebula kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Nebula vårt eget `nebula`-tema.
...
Nebula kommer med flere innebygde temaer.
...
- Vil at Nebula skal matche terminalens utseende
...
Nebula støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/nebula/themes/*.json` eller `$XDG_CONFIG_HOME/nebula/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.nebula/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.nebula/themes/*.json`
