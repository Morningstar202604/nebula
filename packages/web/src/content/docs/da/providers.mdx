---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Nebula.
---

import config from "../../../../config.mjs"
export const console = config.console

Nebula bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Nebula-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/nebula/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Nebula-konfigurationen.
...
Nebula Zen er en liste over modeller leveret af Nebula-teamet, som er blevet testet og verificeret til at fungere godt med Nebula. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Nebula Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg nebula og gå til [nebula.ai/auth](https://nebula.ai/auth).
...
Det fungerer som alle andre udbydere i Nebula og er helt valgfrit at bruge.

---

## Nebula Go

Nebula Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Nebula-teamet, som er testet og verificeret til at fungere godt med Nebula.

1. Kør kommandoen `/connect` i TUI, vælg `Nebula Go`, og gå til [nebula.ai/auth](https://nebula.ai/zen).
...
Sådan bruger du Amazon Bedrock med Nebula:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører nebula:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `nebula.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Nebula understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere nebula til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at nebula skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem nebula-konfigurationen.
...
gemme tokenet i nebula auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Nebula bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Nebula
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`nebula.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Sådan bruger du GitHub Copilot-abonnementet med Nebula:
...
Sådan bruger du Google Vertex AI med Nebula:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem nebula, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [nebula-helicone-session](https://github.com/H2Shami/nebula-helicone-session) plugin for automatisk at logge hver Nebula-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Nebula-samtale opført som en separat session.
...
Du kan konfigurere nebula til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere nebula til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere nebula til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Nebula. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/nebula) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Nebula:
...
3. Kopier API-nøglen til brug i Nebula.
...
6. **Vigtigt**: Før du bruger skymodeller i Nebula, skal du hente modelinformationen lokalt:
...
Nebula Zen er en liste over testede og verificerede modeller leveret af Nebula-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Nebula Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Nebula Zen**.

   ```txt
   /connect
   ```

3. Indtast Nebula API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem nebula-konfigurationen.
...
5. Du kan også tilpasse dem gennem nebula-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Nebula:
...
Du kan også tilpasse modeller gennem nebula-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med nebula. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `nebula.json`-filen i projektkataloget:
...
`limit`-felterne lader Nebula forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `nebula auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek nebula-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i nebula-konfigurationen.
