...
You can add external tools to Nebula using the _Model Context Protocol_, or MCP. Nebula supports both local and remote servers.
...
You can define MCP servers in your [Nebula Config](https://nebula.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/nebula` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Nebula automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Nebula will:
...
If the server requires authentication, Nebula will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `nebula mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Nebula will store the tokens securely in `~/.local/share/nebula/mcp-auth.json`.
...
Your MCPs are available as tools in Nebula, alongside built-in tools. So you can manage them through the Nebula config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Nebula to your Sentry account.
