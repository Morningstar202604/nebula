...
const model = "nebula/deepseek-v4-flash"
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with nebula...`)
  ...
  try {
    await $`nebula run -m ${model} ${prompt}`
  } catch (err) {
  ...
  console.log("  Conflicts resolved with nebula")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with nebula...")
