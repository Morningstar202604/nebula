...
import { createNebula } from "@nebula-ai/sdk"
...
`)
  ...
  const nebula = await createNebula({ port: 0 })
  ...
  try {
    ...
    const session = await nebula.client.session.create()
    const result = await nebula.client.session
      .prompt({
