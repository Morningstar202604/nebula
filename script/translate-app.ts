...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "nebula/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".nebula/glossary/zh-cn.md"
  if (locale === "zht") return ".nebula/glossary/zh-tw.md"
  return `.nebula/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("Nebula did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("Nebula returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("Nebula session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel Nebula runs for 'all' (default: 4)
      --model <provider/id>  Nebula model (default: nebula/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running Nebula
...
`)
  ...
  if (failed.length) console.error(`\nNebula failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.NEBULA_DISABLE_PROJECT_CONFIG = "1"
  env.NEBULA_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["nebula", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.NEBULA_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["nebula", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.NEBULA_CONFIG
  delete env.NEBULA_CONFIG_DIR
  delete env.NEBULA_CONFIG_CONTENT
  delete env.NEBULA_PERMISSION
  delete env.NEBULA_AUTO_SHARE
  return env
}
