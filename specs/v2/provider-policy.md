...
Both `action` and `resource` use nebula's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.nebula` files is intentionally deferred until `.nebula` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether nebula permits an operation through its managed execution paths.
