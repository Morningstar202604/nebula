...
The nebula server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@nebula-ai/llm` tool loop after replacing its
...
- plugins get global "nebula" instance like in that post i showed
- nebula instance has stuff like `nebula.session.prompt()` or
  `nebula.tool.register({...})`
...
- decide whether to preserve deprecated `@nebula-ai/llm` orchestration exports
