...
Move the canonical Nebula terminal application from
`packages/nebula/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/nebula`,
`packages/cli`, or `@nebula-ai/core`.

The SDK is the TUI's Nebula boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/nebula`.
- Do not replace private `packages/nebula` imports with `@nebula-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@nebula-ai/tui`.
...
- Nebula domain data displayed by the TUI
...
```text
packages/nebula/src/cli/cmd/tui
```

Its private dependency on `packages/nebula` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/nebula/src/*`.
...
Status: Completed. The private `@nebula-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@nebula-ai/tui`.
...
- The package has no dependency on `nebula`, `@nebula-ai/cli`, or
  `@nebula-ai/core`.
...
primitives now live in `@nebula-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@nebula-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads Nebula globals or process state;
...
Replace process-global Nebula state with resolved TUI inputs.
...
`@nebula-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/nebula` initially.
...
canonical in `@nebula-ai/tui`. Configured references resolve through the new
...
adapters rather than Nebula domain implementations.

Make the SDK the only Nebula domain boundary used by the TUI.
...
- Domain-facing TUI code imports Nebula data and operations only from
  `@nebula-ai/sdk`.
...
`@nebula-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/nebula`.
...
- The package has no imports from `packages/nebula`, `packages/cli`, or
  `@nebula-ai/core`.
...
public `@nebula-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/nebula`.
- Keep the legacy embedded worker and server startup in `packages/nebula`.
- Change those adapters to load config, create transport inputs, and call the
  public `@nebula-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/nebula`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@nebula-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@nebula-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Nebula domain state.
...
cd packages/nebula && bun typecheck
...
rg '@nebula-ai/core|packages/nebula|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/nebula/src packages/nebula/test
```
...
- Build the current-platform `packages/nebula` binary.
