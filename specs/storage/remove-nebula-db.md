# Remove `packages/nebula/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/nebula/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the nebula legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/nebula/src/storage/db.ts` are concentrated in 22 source files:

- `packages/nebula/src/account/repo.ts`
- `packages/nebula/src/cli/cmd/db.ts`
- `packages/nebula/src/cli/cmd/import.ts`
- `packages/nebula/src/cli/cmd/stats.ts`
- `packages/nebula/src/control-plane/workspace.ts`
- `packages/nebula/src/index.ts`
- `packages/nebula/src/node.ts`
- `packages/nebula/src/permission/index.ts`
- `packages/nebula/src/project/project.ts`
- `packages/nebula/src/server/projectors.ts`
- `packages/nebula/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/nebula/src/server/shared/fence.ts`
- `packages/nebula/src/session/message-v2.ts`
- `packages/nebula/src/session/projectors.ts`
- `packages/nebula/src/session/prompt.ts`
- `packages/nebula/src/session/session.ts`
- `packages/nebula/src/session/todo.ts`
- `packages/nebula/src/share/share-next.ts`
- `packages/nebula/src/storage/db.ts`
- `packages/nebula/src/sync/index.ts`
- `packages/nebula/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy nebula database wrapper; `packages/nebula/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the nebula wrapper.
...
Status: Completed. `SyncEvent` and the nebula projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy nebula database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy nebula database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy nebula database wrapper.
...
All migration groups are complete or superseded. `packages/nebula/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No nebula data-migration group remains.

The previous nebula `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate nebula data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/nebula`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/nebula/src`
- `bun typecheck` from `packages/nebula`
- Relevant package tests from `packages/nebula`, not the repo root
